...
check("fallback: caption confidence clears the apply floor (captions are vision-authored)",
  fallback.bedrooms.every((b) => b.confidence >= BEDDING_SCAN_MIN_CONFIDENCE));
...
check("BeddingTab: scan button + apply flow wired through the shared merge",
  tabSrc.includes("bedding-photo-scan") && tabSrc.includes("mergeBeddingScanIntoUnit"));
check("BeddingTab: apply is an explicit click, never an effect",
  !/useEffect\([^)]*applyScan/s.test(tabSrc));
