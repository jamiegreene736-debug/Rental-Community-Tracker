...
- **Mar 7, 2026**: Built Availability Scanner (`/availability-scanner`). Automated weekly system that scans Airbnb + VRBO/Google Hotels for buy-in availability across all 24 multi-unit properties. Groups properties by community (10 unique communities), searches 78 weekly windows (18 months ahead) for each bedroom configuration. Auto-blocks Lodgify calendar when no inventory found. Runs automatically every Monday at 3am via setTimeout scheduler. Database tables: `scanner_runs` (tracks each scan execution), `availability_scans` (per-community-week results with Airbnb/VRBO counts and block status). Backend: `POST /api/scanner/run` (manual trigger), `GET /api/scanner/status`, `GET /api/scanner/runs`, `GET /api/scanner/results`. Frontend: status cards, results table with community/status/run filters, run history tab. Scanner service in `server/availability-scanner.ts`. Navigation: "Availability Scanner" button on dashboard.
