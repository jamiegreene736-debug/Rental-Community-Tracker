...
-   **Compliance & Registration**: Properties in `unit-builder-data.ts` have `taxMapKey` (Hawaii TMK e.g. `4-2-015-008-0001`), `tatLicense` (TAT License e.g. `TA-023-450-1234-01`), and `getLicense` (GET / General Excise Tax License e.g. `GE-023-450-1234-01`) sample values. These display in the builder's Descriptions tab under a "Compliance & Registration" section as a 4-column grid (TMK · GET · TAT · STR Permit) with copy buttons and a "Push Compliance to Guesty" button (`POST /api/builder/push-compliance`) that saves them as `TMK:`, `TAT:`, and `GET:` prefixed tags on the Guesty listing.
