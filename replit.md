...
- **Mar 9, 2026**: Built Availability Scanner (`/availability-scanner`). Automated weekly system that scans Airbnb + VRBO/Google Hotels for buy-in availability across all 24 multi-unit properties. Searches 52 weekly windows (12 months ahead) per property for each bedroom configuration. When no buy-in inventory found, auto-creates blackout blocks on Lodgify calendar via `POST /v1/reservation/booking` API. Fetches all Lodgify properties + room types at scan start for blocking. Search results cached per community+bedrooms+dates to minimize API calls across properties sharing communities. Runs automatically every Monday at 3am via setTimeout scheduler. Database tables: `scanner_runs` (tracks each scan execution), `availability_scans` (per-property-week results with Airbnb/VRBO counts, block status, and Lodgify block IDs). Backend: `POST /api/scanner/run` (manual trigger), `GET /api/scanner/status`, `GET /api/scanner/runs`, `GET /api/scanner/results`. Frontend: status cards, results table with community/status/run filters, Lodgify push status column, run history tab. Scanner service in `server/availability-scanner.ts`. Navigation: "Availability Scanner" button on dashboard.
