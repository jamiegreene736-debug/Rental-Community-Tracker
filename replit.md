...
- **Feb 17, 2026**: Added "Find Best Buy-Ins" recommendation feature to Buy-In Tracker. Pick travel dates and the system ranks all units by profit margin (sell rate minus buy-in cost), prorated across months for seasonal accuracy. Checks availability against existing buy-ins and Lodgify bookings. Shows top 2 recommended units with full cost breakdown, expandable table of all ranked units, and unavailable unit count. API: `GET /api/availability?checkIn=&checkOut=` returns booked units for date range. Frontend: `getAllUnitPricings()` in pricing-data.ts, `BestBuyInFinder` component in buy-in-tracker.tsx.
