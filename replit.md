...
-   **Compliance & Registration**: Properties in `unit-builder-data.ts` have `taxMapKey` (Hawaii TMK e.g. `4-2-015-008-0001`), `tatLicense` (TAT License e.g. `TA-023-450-1234-01`), and `getLicense` (GET / General Excise Tax License e.g. `GE-023-450-1234-01`) sample values. These display in the builder's Descriptions tab under a "Compliance & Registration" section as a 4-column grid (TMK · GET · TAT · STR Permit) with copy buttons and a "Push Compliance to Guesty" button (`POST /api/builder/push-compliance`). The push is dual-path: (1) stores `GET:`, `TAT:`, and `TMK:` as Guesty listing tags (internal reference); (2) writes a structured `=== Hawaii Tax Compliance ===` block into `publicDescription.notes` — the field that VRBO reads for Hawaii license compliance. The route does a GET-after-PUT verification on both tags and notes, returns `{ verified, notesUpdated, savedTags }`. Note: Guesty's VRBO "license requirements" UI section (with specific GET/TAT/Parcel fields) only becomes editable via API once the listing has an active VRBO channel integration (set up via OAuth in Guesty's UI). Until then, tags + notes is the confirmed API-accessible path.
