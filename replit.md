...
- **Mar 30, 2026**: Community Photo Finder tool built (`/community-photo-finder`). Searches Google Images with 3 targeted queries per community (pool/grounds, aerial/exterior, amenities). Filters out unit interiors, Airbnb/VRBO photos, generic images. Validates results contain community name. "Save to Project" button downloads selected photos directly into the correct community folder, replacing existing files. Backend: `GET /api/community-photos/search`, `POST /api/community-photos/save`, `POST /api/community-photos/populate-all`. One-time auto-population run completed: 9/10 communities downloaded 6 new community-specific photos each (Poipu Brenneckes Beachside had no distinct complex photos online). Properties 36 (Southern Dunes, FL) and 37 (Windsor Hills, FL) removed from all tools — home page, unit-builder-data.ts, routes.ts references.
- **Mar 9, 2026**: Built Availability Scanner (`/availability-scanner`). Per-listing scanning: user selects a specific property from dropdown, then scans 52 weekly windows (12 months ahead) for that listing's bedroom configuration on Airbnb + VRBO/Google Hotels. When no buy-in inventory found, auto-creates blackout blocks on Lodgify calendar via `POST /v1/reservation/booking` API. User specifies which Lodgify property receives blackouts via a second dropdown (fetched from `/api/lodgify/properties`). Fetches all Lodgify properties + room types at scan start for blocking. Search results cached per community+bedrooms+dates to minimize API calls across properties sharing communities. Runs automatically every Monday at 3am (all properties) via setTimeout scheduler. Database tables: `scanner_runs` (tracks each scan execution), `availability_scans` (per-property-week results with Airbnb/VRBO counts, block status, and Lodgify block IDs). Backend: `GET /api/scanner/properties` (list scannable listings), `POST /api/scanner/run` (accepts optional propertyId for per-listing scan, optional lodgifyPropertyId for targeted blocking), `GET /api/scanner/status` (includes currentPropertyId), `GET /api/scanner/runs`, `GET /api/scanner/results`. Frontend: property selector dropdown, Lodgify property selector dropdown for blackout targeting, scan button (disabled until selection), progress shows current property name, status cards, results table with community/status/run filters, Lodgify push status column, run history tab. Scanner service in `server/availability-scanner.ts`. Navigation: "Availability Scanner" button on dashboard.
- **Mar 5, 2026**: Updated all property descriptions in `unit-builder-data.ts`: removed "Regency I"/"Regency II" building identifiers from short/long descriptions, removed "Welcome to Unit A/B" and "Welcome to Beach House A/B" specific labels, added surrounding area/attraction information to all longDescriptions (Princeville, Kapaa/Coconut Coast, Poipu, Pili Mai, Kekaha, Keauhou, Florida). All combinedDescriptions already had generic Unit A/B/C labels, disclaimers, and area info.
...
- **Current Schema:** `users` table (UUID PK), `buy_ins` table (serial PK - Airbnb buy-in purchases with property/unit IDs, dates, costs, confirmations), `lodgify_bookings` table (serial PK - synced guest reservations from Lodgify with guest info, dates, amounts, source), `scanner_runs` table (serial PK - tracks each scan execution with timestamps, counts, status), `availability_scans` table (serial PK - per-community-week results with Airbnb/VRBO counts and block status)
