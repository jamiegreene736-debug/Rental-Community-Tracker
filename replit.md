...
- **Feb 24, 2026**: Expanded buy-in search to multi-platform: Airbnb (via SearchAPI.io airbnb engine with structured pricing), VRBO (via SearchAPI.io Google engine with `site:vrbo.com`), and Suite Paradise (via SearchAPI.io Google engine with `site:suite-paradise.com`). All three platforms searched in parallel. Results displayed with platform tabs showing result counts. Airbnb has full structured pricing with profitability analysis; VRBO/Suite Paradise show Google snippet-based results with links. API: `GET /api/vrbo/search?propertyId=&checkIn=&checkOut=` returns both VRBO and Suite Paradise results. Properties 32 & 33 updated from "Kiahuna Plantation" to "Pili Mai" across all data files.
