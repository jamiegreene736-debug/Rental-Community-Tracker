...
> {
  ...
  // CAPTCHA gate. Google's identifier step can serve a "Type the text
  // you hear or see" image CAPTCHA when Railway's browser is not
  // trusted. Automatic solving is no longer wired here. If this appears,
  // fail with a clear diagnostic and
  // tell the operator to refresh trusted cookies/storage.
  for (let captchaAttempt = 0; captchaAttempt < 2; captchaAttempt++) {
    ...
    trace.push({ step: "google-captcha-detected", detail: `attempt ${captchaAttempt + 1}/2` });
    const shot = await saveShot(workPage, "google-captcha-automation-disabled");
    return {
      ok: false,
      error:
        "Google served a 'Type the text you hear or see' CAPTCHA. Automatic CAPTCHA solving is disabled; refresh GUESTY_SESSION_COOKIES + GUESTY_OKTA_TOKEN_STORAGE to skip Google's login challenge.",
      finalUrl: workPage.url(),
      beforeShotUrl: shot,
    };
  }
