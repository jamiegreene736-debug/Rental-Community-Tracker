  ...
  async verify(input: VirtualStagingViewpointVerificationInput): Promise<void> {
    ...
    const requestBody = JSON.stringify({
      ...
      messages: [{
        ...
        content: [
          ...imageContent,
          { type: "text", text: viewpointPrompt(input.requestedDirection, hasPreviousPreview) },
        ],
      }],
      output_config: {
        format: {
          type: "json_schema",
          schema: viewpointOutputSchema(hasPreviousPreview),
        },
      },
    });
    ...
    for (let attempt = 0; attempt < VIEWPOINT_MAX_ATTEMPTS; attempt += 1) {
      ...
      const verdict = parseViewpointVerdict(payload, hasPreviousPreview);
      if (!verdict) {
        throw new VirtualStagingViewpointVerificationUnavailableError(
          "Viewpoint verifier returned an invalid structured response",
        );
      }
      const accepted = verdict.samePhysicalSpace
        && verdict.viewpointChange === "nearby-natural"
        && verdict.naturalParallax
        && !verdict.fakeTransformDetected
        && verdict.architecturePreserved
        && (!hasPreviousPreview || verdict.distinctFromPreviousViewpoint === true);
      const usage = usageFromPayload(payload);
      console.info(`[virtual-staging] ${JSON.stringify({
        event: "viewpoint-verification",
        verifier: this.id,
        model: this.model,
        ...
        viewpointChange: verdict.viewpointChange,
        naturalParallax: verdict.naturalParallax,
        fakeTransformDetected: verdict.fakeTransformDetected,
        architecturePreserved: verdict.architecturePreserved,
        distinctFromPreviousViewpoint: verdict.distinctFromPreviousViewpoint ?? null,
