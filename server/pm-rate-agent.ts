...
async function runAgentLoop(
  ...
  const systemPrompt = [
    ...
    `2. CHECK WHAT'S ON SCREEN FIRST. Many PM sites land on a non-booking tab by default (Reviews, Photos, Gallery, Description, Amenities, Map, Policies, About). If the visible content is reviews, photos, descriptions, or any non-booking content rather than a date picker / calendar / availability widget / "Book Now" button:`,
    `   a) Look for a tab, button, or link with text like "Availability", "Calendar", "Book", "Book Now", "Reserve", "Check Rates", "Rates", "Dates", or "Check Availability" — click it.`,
    `   b) If no tab is obvious, scroll DOWN — the booking widget may be further down the page below the photos/description.`,
    `   c) If still nothing, look in the page header / nav bar for a "Book" or "Reservation" link.`,
    `   The page MUST show a date picker, calendar, or rate-display widget before you proceed to step 3. Do not skip this check — extracting price from a reviews tab is impossible.`,
    `3. Open the date picker. Then in PRIORITY ORDER:`,
    ...
    `4. Once on the right month, click ${opts.checkIn} as check-in. Then ${opts.checkOut} as check-out.`,
    `5. Click Search / Check Availability / View Rates / Book Now.`,
    `6. Wait for the rate to render. Scroll if the price is below the fold.`,
    ``,
    ...
    `- "DONE: no booking widget" if you searched the page and there is genuinely no calendar / date picker / book button anywhere (rare — most PM unit pages have one somewhere).`,
    ``,
