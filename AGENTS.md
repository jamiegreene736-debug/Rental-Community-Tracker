...
14. **Weekly auto-audit cron (`server/unit-audit-scheduler.ts`)** — the
   ...
   spent) and the REPLACEMENT rung stays OFF unless
   `UNIT_AUDIT_CRON_REPLACE=1` (an unattended job must not silently swap a
   unit; the receipt names the unit needing the one-click replace
   instead). Kill: `UNIT_AUDIT_AUTO_DISABLED=1`. Manual trigger:
