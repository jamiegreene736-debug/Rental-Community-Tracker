...
2. NO FALLBACK to different-listing discovery — that silent substitution is
   ...
   falls back to the manual Replace-with-URL panel when the property has no
   community folder). HONESTY RULE: the flag is set only when the search
   actually COMPLETED (no anchor on file / no same-unit candidates / every
   candidate checked and rejected) — transient infra (SERP quota blackout,
   zero query responses) fails WITHOUT the flag so an outage can never push
