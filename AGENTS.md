...
- **FIND-ONLY, forever.** The brief is `buildCoworkBuyInPrompt` with the `headlessRun` opt
  (default output byte-identical — same lock pattern as bulkBrief); it ends at ATTACH. Never
  wire this runner to the checkout brief — the Cowork send press there is the money approval,
  and payment stays human-only.
- **The agent never holds ADMIN_SECRET.** Its only two portal writes go through the run-scoped
  token proxies `/api/claude-find-runs/agent/:id/{buy-ins,attach}` (auth-excluded prefix,
  timing-safe token check in-handler; the token dies with the run). The proxies PIN
  propertyId/reservation/dates from the run record and reject airbnb.* hosts, then forward via
  127.0.0.1 loopback to the real endpoints — buy-in logic exists once. Blast radius of a
  prompt-injected agent = attach buy-ins to its own reservation, same as Cowork today.
