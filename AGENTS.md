...
6. **Automatic complaint scanner (2026-07-09).** `server/guest-complaint-scanner.ts`
   ...
   (pages the whole inbox ASCENDING/stable, bounded `GUEST_COMPLAINT_BACKFILL_DAYS`=365)
   then INCREMENTAL (only threads/posts past the watermark). LOAD-BEARING invariants — do
   not "simplify": (a) IDEMPOTENCY is a per-message marker `[msg:<postIso>]` stamped on the
