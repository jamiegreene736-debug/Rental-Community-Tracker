...
FOLLOW-UPS (deliberately NOT shipped here — they need live SearchAPI data to tune
safely, and a wrong tweak risks double-listings): (a) the OTA matcher's
**letter-branch** (`hitMatchesUnit`, non-numeric codes like C1/I4) tests a bare
`\bcode\b` over title+snippet+link, so a VRBO **roundup** snippet enumerating
several codes can false-`found` a genuinely-clean letter-coded unit — tighten it
to a unit/resort-context anchor mirroring the numeric branch, leaving the numeric
branch (Poipu Kai) untouched; (b) genuinely-clean off-market 3BR units (e.g. A4,
I4 — confirmed not on any OTA) die at the photo-count/vision gate because
off-market Zillow pages carry few interior photos — handle the sparse-photo case.
Both are tracked but unverifiable without running the live search.
