...
2026-07-20 · Jamie (screenshot): "The alias for both units is still the same. Please change the alias for unit B so it's different from unit A" · SHIPPED (`claude/unit-b-distinct-alias`) · The stored aliases WERE distinct but only by the trailing unit letter (jacelyn.tsu.ae9958.a@ vs .b@) — visually identical. aliasPrefixCandidates now LEADS unit-scoped prefixes with the unit token (unit.b.jacelyn.tsu.ae9958@…), self-describing + at-a-glance distinct; reservation-level aliases keep the historical guest-first base. Locked in tests/simplelogin-alias-prefix.test.ts. Live: unit B's alias row re-minted onto the new shape (unbooked, zero emails — safe); unit A's booked alias untouched.
```
