...
8. **Auto-hide is scoped to HASH-proven duplicate groups (exact/near) —
   AI "same-scene" groups are NEVER auto-applied** (Load-Bearing #4 forbids
   automatic photo dropping without review; the hash classes are the
   operator-approved exception from the audit plan). Pure
...
10. Remaining planned follow-up (PR 3): the photo fix ladder (re-scrape →
   find-new-source → replace-unit, bounded) for community/bedroom/OTA
   failures + the bulk "Audit selected" queue.
