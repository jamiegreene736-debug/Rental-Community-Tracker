...
**No modal on the happy path; the launcher is CAP-FIRST (2026-07-19).** Clicking any Cowork
button copies + fires the deep link + toasts, and opens NOTHING else. The single page-level
paste-fallback dialog opens only when `coworkLaunchNeedsFallback()` says the brief did not
reach the composer (`!launched || !promptIncluded` — phone, or an over-cap prompt whose relay
failed). Keep BOTH terms in that predicate. `launchCoworkPrompt` tries the DIRECT embed first
and reaches for the `cowork_prompt_runs` relay only on overflow: the find brief measures
~14.3k against the 14,336 cap and goes OVER for longer property names, where the old
always-direct path silently opened Cowork with an EMPTY composer. A consequence worth keeping:
the checkout brief (13.6k) now embeds DIRECTLY instead of always relaying, so the operator can
actually read the money brief in the composer instead of a ~400-char bootstrap. Bulk still
always relays (26KB–185KB) and is the only flow with a "View prompt" affordance.

**NEVER live-fire test prompts into the operator's Cowork (2026-07-19).** The 2026-07-13
deep-link verification manually ran `open "claude://cowork/new?q=Deep-link plumbing test…"`
(plus a LENGTH TEST link) on the operator's Mac. Because two deep links fired without a send in
between, both prompts landed CONCATENATED in one unsent composer, and six days later the
operator found the stale "Rental portal deep-link test" task, sent it, watched Cowork run a
"test", and reasonably concluded the portal was pushing tests instead of real briefs (it never
was — the deployed bundle greps clean, verified live). The rule: dev sessions verify deep-link
plumbing via the unit suite, bundle greps, and reading the app's URL handler — never by firing
`claude://` links carrying test content at the operator's Claude Desktop. If a live fire is
ever truly unavoidable, get the operator's explicit go-ahead first and delete the resulting
Cowork task in the same session. Enforced in code where possible:
`tests/cowork-launch.test.ts` scans all shipped source (client/server/shared/daemon/scripts)
and fails on any dev-test prompt marker or any hard-coded prompt after `cowork/new?q=` — the
only sanctioned prompt carrier is `buildCoworkDeepLink()`'s dynamic `encodeURIComponent`.

**Cowork tasks CANNOT be auto-started from the portal (2026-07-19, verified — do not re-chase).**
Claude Desktop 1.22209.3's `claude://` handler (fn `Dye`, `.vite/build/index.chunk-rKZpGjtP.js`)
builds a FRESH `URLSearchParams` and forwards only `q`/`folder`/`file` (+`src=external`) to the
REMOTE `https://claude.ai/task/new`; no parameter can mean "and run it", and every other host
branch is more restrictive. There is no `bin`, no `second-instance` handler and no listening
socket, and eIPC is origin-gated to claude.ai. Anthropic's real auto-start channel
(`/v1/dispatch/host/events` → `startSession`) is cloud-side and account-scoped — unreachable
from a web page. An osascript/System-Events keystroke via the local sidecar was considered and
REJECTED: Claude Desktop ships no `.sdef`, so it would be blind UI scripting from a launchd
agent, TCC-fragile, racing our own focus-guard, with a blast radius that includes destructive
`window.confirm` dialogs and the guest inbox send box. Pressing send stays the human approval.

**Headless Claude find-runs — the zero-click alternative to the Cowork find button (2026-07-19).**
Because Cowork can't be auto-started, the operator chose a headless runner for the FIND flow:
"Auto-run · find cheapest (no window)" on the bookings row POSTs the same CoworkBuyInPromptInput
to `POST /api/claude-find-runs`; the Mac sidecar daemon's runner child
(`daemon/vrbo-sidecar/claude-find-runner.mjs`, spawned by worker.mjs on the LOCAL role slot 1
ONLY — never Railway server workers) claims it via X-Admin-Secret and executes the brief with
`claude -p` — no window, no send press. Load-bearing boundaries, all test-locked in
tests/claude-find-run.test.ts:
- **TWO RUN KINDS; payment stays human-only.** (REPLACES the original "FIND-ONLY, forever"
  rule — operator directive 2026-07-20, per-unit checkout buttons must "execute cowork
  automatically"; see that Decision Log line.) kind `"find"` = `buildCoworkBuyInPrompt` +
  `headlessRun`, ends at ATTACH. kind `"checkout"` = `buildCoworkCheckoutPrompt` +
  `headlessRun` for exactly ONE pinned buy-in (`POST /api/claude-find-runs/checkout` reads the
  AUTHORITATIVE buy_ins row via loopback and gates on `checkoutRunEligibility` — attached to
  this reservation, unbooked, no active claim, https vrbo.com URL, costPaid > 0 — so the brief's
  money anchors never come from a client screen). The old FIND-ONLY rule's money gate was the
  Cowork send press; the gate is now the CARD ITSELF: a checkout run claims the reservation's
  one checkout lane, prepares the VRBO page, records awaiting_payment, prints an
  `ATTENTION: awaiting payment` line, leaves the prepared tab OPEN, and ENDS. It never enters
  card data, never clicks the final purchase control, and never writes booked/request_submitted
  — the operator pays in the surfaced tab and records the outcome with the row's
  "Paid — mark booked" control (or "Not paid — reset"). Both defaults (no-opts find AND
  checkout briefs) stay byte-identical — test-locked.
- **The agent never holds ADMIN_SECRET.** Every portal write goes through run-scoped token
  proxies under `/api/claude-find-runs/agent/:id/` (auth-excluded prefix, timing-safe token
  check in-handler; the token dies with the run). Find runs get `{buy-ins,attach}` (pin
  propertyId/reservation/dates, reject airbnb.* hosts, REQUIRE costPaid > 0); checkout runs get
  `{buy-in (GET), traveler-email, checkout-claim, checkout-claim/complete,
  checkout-claim/release}` — all pinned to the run's reservation AND its ONE buyInId, and there
  is deliberately NO proxy that can PATCH a bookingStatus. Everything forwards via 127.0.0.1
  loopback to the real endpoints — the logic exists once. Blast radius of a prompt-injected
  agent = attach buy-ins to / prepare the checkout lane of its own reservation.
- **Tool allowlist is the capability surface**: `mcp__chrome` (chrome-devtools-mcp attached to a
  DEDICATED hidden persistent-profile Chrome on CDP 9250 — never the operator's personal Chrome,
  never the 8 sidecar scrape instances), `Bash(curl:*)`, WebSearch, WebFetch. No bare Bash.
- **Feedback is portal-native**: the runner relays stream events (token-scrubbed) to the run
  store (`claude_find_runs.v1`, promise-tail); the row panel renders live narration, amber
  ATTENTION states (the brief prints `ATTENTION:`/`RESUMED:` marker lines; the wrapper chimes
  Sosumi locally), and the agent's final message as the persisted report. Server watchdog closes
  orphans honestly (Mac offline / silent / 90-min ceiling).
- **One-time Mac setup**: the `claude` CLI login is separate from Claude Desktop — run `claude`
  once in Terminal and `/login` (or set CLAUDE_FIND_RUN_API_KEY in the sidecar env). Until then
  runs fail fast with exactly that instruction. Kill switch: CLAUDE_FIND_RUNS_DISABLED=1 (server
  create + daemon spawn). The LIVE daemon copy is ahead of main — apply worker.mjs deltas
  surgically + `launchctl kickstart -k gui/$(id -u)/com.vrbosidecar.worker`.
- **Runner Chrome surfaces itself on ATTENTION** (2026-07-20): the dedicated Chrome launches
  minimized/off-screen, so whenever the run raises attention (bot wall, awaiting-payment
  handoff) the wrapper's `surfaceRunnerChrome()` brings the window on-screen via CDP
  `Browser.setWindowBounds` (global WebSocket, chrome-sidecar-manager's technique). Best-effort,
  never yanked back off-screen — the operator may be mid-use. The PAYMENT handoff specifically
  gets `surfaceCheckoutHandoff(reason)` — the sidecar's YELLOW challenge treatment painted onto
  the prepared checkout tab (top banner "💳 ADD THE CREDIT CARD … No purchase has been
  submitted" + click-transparent yellow border, ids `rct-findrun-card-*`), tab fronted via
  `/json/activate`, near-fullscreen window. Routing is the `/^awaiting payment\b/i` match on the
  attention reason, DRIFT-LOCKED behaviorally against the brief's mandated
  "ATTENTION: awaiting payment — …" line in tests/claude-find-run.test.ts. DISPLAY-ONLY is
  load-bearing: the injection never touches the payment form, prefills nothing, intercepts no
  clicks (test-locked); the banner disappears naturally on the operator's own Checkout
  navigation.
- **"Auto checkout" per-unit buttons** (`HeadlessCheckoutRunButton`, one per attached unbooked
  unit) start kind-"checkout" runs of `buildCoworkCheckoutPrompt` — CHECKOUT PREPARATION ONLY,
  on the attached (operator-reviewed) units. The BULK "Prepare all checkouts" button still
  deep-links the combined Cowork brief (one deliberate card sitting).
...
GONE (2026-07-19).** Each attached slot renders `UnitPurchaseStatus` (bookings.tsx, was
`BuyThisUnitInButton`): the durable `buy_ins.bookingStatus` badges plus ONE recorder —
`markBookedAfterPayment` (from the same-day headless-checkout PR #1115: prompts for the real
confirmation number, "REQUEST" → request_submitted, blank → confirm-gated bare booked) —
PATCHing `/api/buy-ins/:id`, which arms the never-re-book guard and hides the unit's checkout
button. The `awaiting_payment` handoff has BOTH honest exits side by side: "Paid — mark
booked" and "Not paid — reset"; `request_submitted`, `failed`, and idle rows get a "Mark as
bought in" wrapper around the SAME recorder (don't fork a second PATCH path). The per-unit
"Buy this unit in" button (the dormant sidecar checkout job's only UI trigger) was REMOVED at
the operator's direction — checkout preparation is Cowork-only. The server scaffold
...
**AUTO-mark on alias email (2026-07-20, operator: "any email hitting is verification").** Any
INBOUND email at a unit's SimpleLogin alias flips the owning buy-in to "booked"
automatically — the alias exists only as that unit's booking email, so VRBO confirmations /
host welcomes / PM arrival mail all follow a real purchase. ONE marker
(`markBuyInBoughtInFromInboundEmail`, server/guest-inbox-sync.ts; pure decision
`aliasEmailProvesPurchase` in shared/alias-bought-in.ts) fires at BOTH ingestion seams
(guest_inbox_messages import + buy_in_emails insert, i.e. both 5-min IMAP ticks) AND
reconciles on both panel reads (GET /api/guest-inbox + GET …/buy-in-communications — this is
what heals units whose emails were ingested before the feature shipped; responses carry
`autoMarkedBoughtIn` / `autoMarkedBuyInIds` so the client refreshes the rows + toasts).
Rules, test-locked in tests/alias-bought-in.test.ts: inbound only (our outbound sends prove
nothing), never re-marks a booked row (bookedAt stays the FIRST proof; set explicitly —
storage.updateBuyIn bypasses the PATCH handler's auto-stamp), cancelled buy-ins left alone.
Kill: ALIAS_AUTO_BOUGHT_IN_DISABLED=1. The bookings-row "Mark as bought in" button stays for
units bought outside the alias flows.
...
### Find-unit sibling street-root RESCUE (2026-07-18)

The `/api/replacement/find-unit` resort-street gate rejects every
candidate whose URL street root doesn't match the community's
configured/curated root(s). When the CONFIGURED street NUMBER is wrong
(a directory/maps address no listing portal indexes under — live case:
Wavecrest Resort's draft carried "8001 Kamehameha V Hwy" while every
Zillow/Redfin unit lives at 7142/7144/7146/7148 Kamehameha V Hwy), the
gate strikes out 100% and the search dies with "all N filtered out".
The rescue (`shared/discovery-root-rescue.ts`, wired in routes.ts right
after the main SearchAPI discovery loop) learns the resort's REAL
building roots from the REJECTED results and replays them. Its four
precision rails are each load-bearing — don't loosen any of them:

1. **Strikeout-only** (`rootGateAdmissions === 0`): fires only when the
   gate admitted NOTHING. A merely-noisy result set never widens the gate.
2. **Same street name+type**: a learned root differs from a configured
   root ONLY in the leading street number(s). No street jumps, ever.
3. **Contiguous community-name anchor**: the rejected result's
   title/snippet must contain the community name (or curated alias) as a
   contiguous phrase after generic-token folding. Token-SET matching is
   forbidden — a "Waikoloa Colony Villas … Waikoloa Beach Dr" snippet
   contains every token of "Waikoloa Beach Villas" without being that
   resort. Anchor phrases contained in the resort's own street name are
   dropped entirely (a community literally named after its street can
   never anchor, so the rescue stays silent there).
4. **Recurrence ≥2 distinct listings** per learned root, and streets in
   `HAWAII_STREETS_WITH_DISTINCT_RESORTS_BY_LOT` are excluded wholesale
   (there the lot number IS the resort identity).

Learned roots are added to `communityAddressRoots`/`directAllowedRoots`
in place so the second-wave street-root queries and the Apify/RealtyAPI
legs inherit them. Candidates admitted via a learned root still pass
every downstream gate (bedroom, OTA-clean, Claude-vision community
check, photo floor) before anything is committed. The durable fix for a
confirmed-wrong street remains a curated `CommunityAddressRule` with
`buildingStreetRoots` (Wavecrest now has one — rule street 7142
Kamehameha V Hwy, buildings 7142/7144/7146/7148 + the Zillow /b/ page);
the rescue is the self-healing layer for communities nobody has curated
...
(`nexstay_bedding_<propertyId>`), but a unit's bedroom COUNT is a
listing-level fact owned upstream — `resolveDraftUnitBedrooms` over the
`community_drafts` row for drafts, `unit-builder-data.ts` for static
properties. That count drives the title, pricing, descriptions, and the
audit's layout stage, so the tab must describe it, not out-vote it.

1. **`normalizeUnit` reconciles on every load.** `loadBeddingConfig`
   merges stored-over-default by `unitId`;
   `reconcileUnitBedroomSlots` (pure,
   `shared/bedding-bedroom-reconcile.ts`) then corrects the stored
   array's LENGTH against the freshly-rebuilt default. Operator/scan
   edits on the surviving slots are preserved verbatim — only the count
   moves. Excess slots drop from the END (deterministic, and it mirrors
   how `buildDefaultUnitBedding` pads and how the tab's `removeBedroom`
   renumbers); short lists top up from the defaults; `roomNumber` is
   renumbered 1..N because `buildGuestyListingRooms` and the tab's
   per-bedroom mutators key on it. **Do not "simplify" this back to
   taking `stored.bedrooms` wholesale** — that is the exact 2026-07-18
   Cliffs at Princeville regression (see the Decision Log): draft 20's
   `unit1Bedrooms` was corrected 3 → 2, the tab kept its third slot, and
   the live listing was pushed as a 7BR sleeping 18 under its own
   "6BR for 16" title.

2. **The Guesty push is guarded.** `totalBedrooms(config)` — the length
   of this browser's slot arrays — is the ONLY bedroom count the Guesty
   listing ever receives; the proxy's `classifyGuestyProxyListingWrite`
   seam is explicitly display-only and validates nothing.
   `blockedBeddingPushReason` therefore refuses any push whose count
   contradicts `authoritativeTotalBedrooms(propertyId)`. It sits in
   `pushBeddingConfigToGuesty`, which is the single write path shared by
   the manual "Push Bedding to Guesty" button AND the photo scan's
   auto-push — keep it that way; a second `updateListingDetails` call
   site would route around the guard (test-locked).

3. **Two honesty rules, both test-locked.** An EMPTY authoritative list
   means *unknown*, never *zero bedrooms* — a draft whose row hasn't
   loaded must not empty a real config or push `bedrooms: undefined`
   with an empty `listingRooms`; and likewise `blockedBeddingPushReason`
   never blocks on an unknown (0) count. A reconciliation is REPORTED,
   never silent: dropping a slot may be discarding a hand-added bedroom,
   so the tab renders an amber notice naming every unit it corrected.

4. **Bathrooms are deliberately NOT reconciled.** The draft's
   `unit1Bathrooms`/`unit2Bathrooms` are free TEXT, and half-baths are
   legitimately operator-owned in this tab. Only the bedroom count has
   an unambiguous upstream owner.

This composes with — and does not replace — the photo scan's own rule
that "bedroom/bathroom COUNTS never change"
(`shared/bedding-photo-scan.ts`): the scan fills what is IN each
bedroom, this reconciler owns how many there are.
...
`PHOTO_DEDUPE_HASH_DISTANCE`) + batched Claude Sonnet vision calls for
"same scene, different angle" groups (downscaled images, per-call cap
...
The request covers EVERY gallery the tab renders — both unit folders and
the community folder — because it is built from the rendered `photos`
array (see point 7).

**The two signals have DIFFERENT coverage guarantees and must never be
reported as one (2026-07-18).** Hash clustering compares every pair of
every photo, always. The vision pass is the ONLY signal that can catch
"the same tree from another angle" — two angles of one scene sit far
above `NEAR_DUPLICATE_DISTANCE` by construction, so a hash can never
find them — and its coverage is budgeted. Manual Photos-tab scans plan
calls with `buildManualVisionBatchPlan`, three tiers: (1) folder fits
`PHOTO_DEDUPE_VISION_CAP` → one call, every pair compared; (2) exhaustive
pair-cover fits `PHOTO_DEDUPE_MANUAL_MAX_BATCHES` (default 4) →
`buildCompleteVisionBatchPlan`; (3) otherwise DISJOINT chunks so every
photo is seen at least once, reporting `visionComplete:false`. Strict
dashboard automation instead passes `requireCompleteVision` and REFUSES
anything short of full pair coverage (`COMPLETE_VISION_MAX_BATCHES`=12,
exhaustive to ~150 photos). The manual path also carries a scan-wide
wall-clock budget (`PHOTO_DEDUPE_SCAN_BUDGET_MS`, default 8 min) because
it answers one synchronous HTTP request and Railway's edge hard-cuts any
response at 15 minutes; past the budget the remaining folders go
hash-only and say so. THE BUDGET BELONGS TO THE TRANSPORT: it is applied
at the HTTP route, never defaulted inside `scanForDuplicatePhotos` —
background callers (the sweep runs in-process and makes up to four scans
inside one stage) must run unbudgeted, and an earlier cut that defaulted
it in the engine silently truncated the vision leg of every
NON-fullAutomation sweep. Each folder gets a fair share of the REMAINING
budget rather than consuming it in order, because folder-order spending
let the LAST gallery — typically Unit B — get zero repeat-angle coverage,
which is exactly the question this feature has to answer. Folders scan
sequentially and a folder stops before STARTING a batch once its share
has elapsed, so at most one in-flight Claude call (2 min) can overrun:
8 + 2 = 10 min, inside the 15-min cap. Raising the env past ~12 min
reopens that risk. WHAT WAS WRONG BEFORE: the manual path took ONE
evenly-strided sample capped at 60, so a 150-photo gallery showed Claude
60 photos, never looked at the other 90, and the tab still rendered "✓ No
duplicate or redundant photos found across 150 photos". Do NOT
reintroduce a local sampler in `server/photo-dedupe.ts` (test-locked).
...
3. **Vision grouping is conservative by construction, but a suppressed
   "maybe" is now REPORTED instead of dropped (2026-07-18).** Only
   high-confidence vision groups are ever PROPOSED for removal; a vision
   edge is refused when the two photos' effective categories differ or
   ...
   Two changes to how the suppressed cases are handled, because together
   they were silently eating exactly the repeat-angle class this feature
   exists to find: (a) edges are now gated PER PAIR, not in a star from
   `valid[0]` — one blocked first edge used to discard every other pair in
   the group, so a 3-shot repeat lost all of it when the first photo
   carried a different machine category; (b) medium-confidence groups and
   guarded-but-unmerged edges surface as REVIEW groups instead of
   vanishing. An angle variant can never be rescued by the exact-hash
   bypass (two angles are far apart in hash space), so before this the
   guard was a silent dead end.
   (c) THE GUARDS ARE ENFORCED OVER THE WHOLE CLUSTER A UNION WOULD PRODUCE,
   not just its two endpoints (`unionRespectingClusters`, which tracks the set
   of bedroomClusterIds/categories per union-find root). Pair-level gating is
   NOT sufficient and this is the subtlest trap in the file: union-find is
   transitive, and a photo carrying no bedroom cluster (the async labeler has
   not written its row yet — routine) is edge-allowed against BOTH bedroom 1
   and bedroom 2, so unioning those two allowed pairs silently re-merges the
   pair the guard just refused — and the blocked-edge review path then sees
   them already merged and stays quiet. Caught by adversarial review of the
   first cut of this change, where it produced a confirmed group with a second
   bedroom's photo pre-selected for removal that the weekly sweep would
   auto-hide. Regression-locked with a 3-photo bridge test; do NOT "simplify"
   the union back to a bare `union(a, b)` after `dedupeEdgeAllowed`.
4. **The review tier is reported, never proposed, never automatic.**
   `DedupeFolderResult.reviewGroups` is SEPARATE from `.groups` and that
   separation is load-bearing three ways: the unit-audit sweep treats
   `folders[].groups` being empty as "gallery is clean" (putting review
   candidates there would make every weekly sweep flag properties over
   findings it can never act on); every review member carries `keep:true`
   so nothing is pre-selected in the UI; and `dedupeAutoFixSelections`
   skips `kind === "review"` outright, on top of the sweep's positive
   kind partition that already never routes them there. The operator CAN
   still confirm a review photo — `validateDedupeSelection` accepts review
   members and applies the identical keep-one and never-empty-folder
   guards — it just has to be a deliberate click. Note that validator
   counts DISTINCT filenames per folder, because one photo may legitimately
   appear in both tiers.
5. **A completed scan must never claim coverage it did not have.** The
   engine has always computed `visionComplete` / `scannedForVision` /
   `visionEligible`, but nothing consumed them on the manual path, so a
   partially-covered gallery rendered exactly like a fully-covered one.
   Now: `summarizeDedupeFolders` emits a coverage warning whenever
   `!visionComplete` (worded to split the two signals — the hash pass did
   cover everything); the Photos tab renders `warnings` + `note` on EVERY
   completed scan, not only when groups were found (they used to be inside
   the `groupCount > 0` branch, i.e. hidden on exactly the branch where a
   false clean is dangerous); a per-folder coverage receipt renders for
   every scanned gallery including clean ones, which is also the
   operator's proof that BOTH units were scanned; and the green verdict
   only claims repeat-angle coverage when every gallery reports
   `visionComplete`.
6. **Keeper pick is deterministic** (`pickKeeperIndex`): human-touched
...
7. **Client-driven groups, same as the community check** — the request is
...
8. **A confirmed removal propagates to Guesty via the gallery re-push
   ...
   photo stages); the SWEEP syncs at its own seam instead
   (2026-07-15 follow-up): `dedupeHiddenThisSweep` counts the hides
   the dedupe stage applied, and `startSweepDedupeGuestySync` fires
   ONE fire-and-forget `repushGuestyPhotosForProperty` at sweep END —
   after every photo stage AND the retry rails, so the minutes-long
   push can never race the replace rung or the collage pin. Skipped
   when the replace rung committed a swap this sweep (the swap's own
   awaited re-push already drops hidden rows — a second push is pure
   duplicate Guesty load) and when the property isn't Guesty-mapped;
   fires on completed AND failed sweeps (the hides are durable either
   way), never on cancel. RATE-LIMIT POSTURE: self-quenching — a push
   fires only when a hide landed THIS sweep, so the weekly ~31-listing
   cron pushes only where dupes were newly found; sweeps run
   one-at-a-time so pushes space out; Guesty calls ride guesty-sync's
   global gate. Kill `AUDIT_DEDUPE_GUESTY_PUSH=0` (receipt then says
   "NOT removed from Guesty"). The sync verdict is appended to the
   photo-dedupe receipt row; the photos push ledger is the completion
   surface. KNOWN LIMIT (shared with the Photos-tab path): a hide in
   a SHARED community folder re-pushes only the audited/clicked
   property — sibling properties serving the same community folder
   stay stale until their own next push. Test-locked in
   tests/photo-dedupe.test.ts.
...
2. **Both collage endpoints share ONE Guesty tail** —
   ...
   re-inline a copy on either endpoint.
2b. **"pick manually" runs the SAME server composer (2026-07-18 — REPLACES the
   old client-canvas flow).** The picker posts `auto-cover-collage` with an
   explicit `picks: {left:{url}, right:{url}}`; the engine resolves it via
   `resolveForcedCollagePick` and composes EXACTLY that pair —
   `method: "manual"`, no vision call, no spend. **A forced pick that can't be
   resolved (external URL, file missing on disk, same photo twice) THROWS — it
   must NEVER degrade to the vision/heuristic pick.** That silent substitution
   IS the reported bug: the operator picked two photos and got Claude's collage
   back. Source-guarded in `tests/cover-collage-logic.test.ts`.
   The old path (per-pick `/api/builder/upscale-photo` → client canvas →
   `/api/builder/upload-collage`) is GONE and must not be restored: it gated
   Real-ESRGAN on the **1920px push spec (long side)** while a collage panel is
   only **800px (short side)**, so a typical 1280×948 photo burned a ~30s
   Replicate job per pick that the AI path correctly skips — plus an ImgBB
   upload + browser re-download of both source photos just to feed the canvas.
   Manual generation was minutes slower than one-click AI and read as broken.
   `/api/builder/upload-collage` remains registered (shared-tail chokepoint) but
   the builder no longer calls it.
...
10. **Availability scheduling is explicit opt-in.** Opening the
    Availability tab must never enable or re-enable a scheduler row. The
    existing Enable/Disable control is the only client-side path that changes
    `enabled`; rows created while saving other settings default to disabled.
    This intentionally reverses the auto-enable behavior from PRs #14/#17/#18.
...
53. **The guest inbox spans TWO systems; "both ways for every channel" hardening
    ...
    - **System B is now TWO-WAY:** `POST /api/guest-inbox/send` replies to the host
      ...
      `sendBuyInEmail`, writes a `direction:"outbound"` row); the composer +
      direction-aware bubbles now live inside the MERGED `BuyInVendorEmailPanel`
      (`bookings.tsx`) — `BuyInGuestThreadPanel` was folded into it 2026-07-19 (see
      the next bullet). The reply target is
    ...
    - **ONE ALIAS PER ATTACHED UNIT (2026-07-19, operator-reported duplicate).**
      The VRBO traveler/booking email IS the unit-scoped `(reservation_id, buy_in_id)`
      alias from `server/reservation-alias.ts` (extracted VERBATIM from routes.ts).
      `ensureTravelerEmailForBuyIn` (buy-in-checkout-job.ts) mints/reuses THAT alias —
      the legacy per-guest `firstname.lastname@emailprivaccy.com` scheme is DELETED
      because its unit-index disambiguation raced sibling attaches and its
      "already exists" catch silently stored ONE address on two units. Rules, all
      test-locked (`tests/unified-buyin-alias.test.ts` + the pipeline-logic lock):
      (a) minting/looking up a unit alias BACKFILLS `buy_ins.travelerEmail` but NEVER
      clobbers an existing value; (b) an UNBOOKED unit whose travelerEmail is shared
      with a sibling re-mints onto its own alias (`travelerEmailNeedsRemint`,
      shared/unified-buyin-alias.ts) — a BOOKED unit keeps its address because VRBO
      already has it; (c) the bookings row renders ONE email block per unit
      (`BuyInVendorEmailPanel`): since BOTH IMAP ingesters (buy_in_emails +
      guest_inbox_messages) watch the same mailbox filtered by the same address,
      the panel renders `mergeAliasThread()` — a Message-ID/subject+sender+window
      deduped union — never two parallel histories; (d) a legacy travelerEmail that
      differs from the unit alias stays VISIBLE ("booking email (legacy)" badge) —
      VRBO still mails it.
...
33. **Single-listing drafts share the `community_drafts` table with
    ...
    **Generator grounding (2026-07-17).** `generate-listing` grounds
    ...
    early return so the fallback path never touches storage.
    Audit rules that must not drift (each fixed a confirmed review
    finding): confirmed strings are matched on their BED PORTION only
    (cut at "Bathrooms:" PER UNIT, array form — bathroom text like
    "Double vanity"/"Full Bath" must never satisfy a bed regex);
    confirmed regexes stay plural-tolerant ("2 Kings"); the model's
    `bedding` field is never audited (it is overwritten); a corrective
    retry is adopted only when `generatedDraftCompletenessRegressions`
    is empty (an audit-clean retry that dropped unitA would otherwise
    ship fallback filler); and `describeUnitBedding` humanizes bed
    types outside GuestyBedType (draft-bedding.ts emits TWIN_BED etc. —
    "undefined" must never reach the authoritative string). Locked by
    tests/description-copy.test.ts + tests/bedding-space-copy.test.ts.

    **Every summary EXPLAINS its own occupancy (2026-07-18).** Guests
    kept asking "how does a 4 bedroom sleep 12?", so every generated
    summary now shows the arithmetic: bedrooms at 2 each, plus the
    sleeper sofa in each unit's living area. That math is NOT new —
    `occupancyForBedrooms` has always been "2 per bedroom + sleeper
    sofas" (+2 single condo, +4 two-condo combo).
    `buildSleepingCapacityExplanation` (shared/description-copy.ts)
    DERIVES the paragraph from that one rule, so it can never disagree
    with the title's "Sleeps N", the dashboard Guests column, or Guesty
    `accommodates` — **never hand-write these numbers or re-derive them
    from bed counts.** BELT AND BRACES: both prompt variants get the
    math as authoritative CONTEXT plus `SLEEPING_CAPACITY_RULE` (so
    Claude writes it in its own voice), AND the endpoint
    deterministically ensures it after parsing — "all listings explain
    this" must not depend on model compliance. The ensure runs BEFORE
    the resort-fee append (the fee caveat stays last) and on the no-key
    FALLBACK path too. This single seam covers the Descriptions-tab
    button, the audit sweep's regenerate twin, and the bulk-combo
    pipeline; don't add a second. HONESTY RULES, each of which returns
    null / says NOTHING rather than assert: 0 bedrooms; a non-even sofa
    remainder; and a STANDALONE listing carrying the combo +4 (that
    would claim two sleeper sofas in one living room). Sleeper sofas are
    described WITHOUT a size — the Bedding tab captures only a count, so
    "queen sleeper sofa" is an invented fact (the same rule
    `buildSpaceDescription` follows). Placement is disclosure-aware:
    `ensureSleepingCapacityExplanation` targets the last NON-disclosure
    block (`SUMMARY_DISCLOSURE_LEADS`) so the paragraph joins the
    summary BODY instead of landing under the "Unit assignment note"
    legalese — reword a disclosure in unit-builder-data.ts and update
    that lead list in the same PR (test-locked). Detection
    (`describesSleepingCapacity`) accepts any wording naming a sofa bed
    plus both numbers — a false negative would append a DUPLICATE
    explanation to live guest copy — but all three signals must land in
    the SAME PARAGRAPH. Whole-summary matching was too loose against
    real data: a live 5BR listing matched a "sofa bed" in its Unit A
    blurb, "14 guests" up top, and the 10 from "a short 10-minute walk"
    three paragraphs down, so the backfill skipped a listing that had no
    explanation at all. Don't relax this back to a whole-text scan.

    **Backfill is SURGICAL, not a re-generation (2026-07-18, operator's
    explicit choice).** `POST /api/admin/backfill-sleeping-capacity`
    takes each mapped listing's CURRENT Guesty summary, inserts the one
    paragraph, and pushes it back — it makes NO Claude call
    (test-locked). A full AI regenerate would rewrite good live OTA copy
    and clobber hand-edited Descriptions-tab overrides for no added
    benefit. It persists the override BEFORE the Guesty PUT (order is
    load-bearing: the override table is what a later push-descriptions
    sends, so skipping it would let the next push silently revert
    Guesty), read-back verifies, and stamps the push ledger. Dry-run by
    DEFAULT (`{ execute: true }` to write) like
    reflow-description-disclaimers, because it mutates live OTA
    listings; NDJSON + heartbeat because a serialized walk behind a
    Guesty 429 pause can outlive Railway's 15-min edge cap. It SKIPS
    (never "fixes") a listing whose Guesty `accommodates` disagrees with
    the computed occupancy — that is real bedroom-count drift (the
    2026-07-18 Cliffs class) and needs a human, not a paragraph arguing
    with the listing's own headline.
...
gallery Lens, `maxPhotos: PHOTO_AUDIT_MAX_PHOTOS`). The decision
...
2. **Why it exists on top of the discovery-time OTA preflight:** the preflight
   is a 3-photo Lens + address screen per CANDIDATE (candidate-side sourcing
   hygiene — unrelated to the removed our-unit address-theft leg); this gate
   scans the FULL persisted gallery of the CHOSEN units, so a repost that only
   copied deeper gallery photos still blocks publish. It also writes the normal
...
### Photo OTA detection audit (`server/photo-listing-scanner.ts`)

The recurring "is this unit listed on Airbnb/VRBO/Booking?" audit (dashboard
Photos column + weekly cron). One leg per scanned UNIT folder (community-*
folders are skipped — no unit signal):
...
2. **Address leg — REMOVED 2026-07-18** (was: one `site:host "street" "city"`
   SERP per platform, added 2026-06-29 with deep-fetch/geo/backfill phases).
   The separate published address (2026-07-17) puts the community CLUBHOUSE /
   generic street on every listing, so our real unit addresses no longer
   appear on our own listings and the "someone listed our unit's address"
   signal is moot — operator directive 2026-07-18 (see that Decision Log
   line). Gone with it: the 4 `photo_listing_checks` `*_address_status` /
   `address_matches` columns (schema-maintenance DROPs them on boot — do not
   re-ADD), `shared/address-listing-logic.ts` / `address-page-match.ts` /
   `address-geo-match.ts` / `address-alert-warning.ts`, the
   `POST /api/photo-listing-check/address-backfill` endpoint, the dashboard 📍
   mini-row + address-alert popup, the address-found SMS, and the
   `PHOTO_LISTING_ADDRESS_*` env knobs. The generic `parseStreetCityState`
   parser SURVIVES in `shared/address-parse.ts` (published-address +
   audit/replace target resolution import it — unrelated to detection). The
   candidate-side OTA screens (`server/combo-ota-preflight.ts`,
   `runOtaQualifier`) keep their OWN address searches — those qualify OTHER
   people's units as sourcing candidates and are a different system.
...
- **The dashboard "Run photo match scan" button (`POST /api/photo-listing-check/run`)
  ...
  + per-folder photo status dots. Progress is DERIVED client-side (no
  server job state) — reliable because scans run sequentially and upsert
  `checkedAt` per folder.
- **Outage preservation** (`persist`): an inconclusive
...
- **Balanced multi-photo agreement (photo leg, 2026-06-29).** Baseline `found` is
  ...
  top of #858's deep-gallery audit — don't confuse it with the deep-gallery
  depth itself.**
- Toggles: `PHOTO_LISTING_LENS_DISABLED` (photo leg),
  `PHOTO_LISTING_SCAN_INTERVAL_DAYS` (cadence, default 7).
- (The `folderAddressContext` resolution + address-only backfill bullets that
  lived here were REMOVED 2026-07-18 with the address leg — see leg 2 above.
  `parseStreetCityState` lives on in `shared/address-parse.ts`.)
- **FOUND-FLIP REACTION (2026-07-12): the weekly scheduler closes the detect→fix gap itself.**
  ...
  `PHOTO_FOUND_AUTO_AUDIT_DISABLED=1` (alerts still send).
...
- **Operator SMS alerts (`server/operator-alerts.ts`, 2026-07-12).** Proactive channel for
  ...
  flips (with the queued-sweep outcome) and cron
...
### Booking rules (advance notice) auto-push at mapping birth (Load-Bearing, 2026-07-19)

The 7-day advance-notice cutoff lives in `builder_booking_rules` +
Guesty's `PUT /listings/:id/availability-settings`
(`calendarRules.advanceNotice.defaultSettings.hours`). Three rules,
born from the Royal Kahana incident (VRBO sold a next-day arrival on
2026-07-19 because the listing — mapped after the one-and-only
2026-06-02 `push-advance-notice-all` sweep — had NO cutoff in Guesty):

1. **Every mapping-birth seam fires `autoPushBookingRulesForMapping`**
   (routes.ts, beside `autoPushSavedAmenitiesForProperty` /
   `ensurePublishedAddressForMapping` at all five seams). It SKIPS when
   a saved `builder_booking_rules` row exists (operator rules are never
   clobbered), merges the listing's LIVE Guesty terms (a curated
   maxNights survives), forces the standing 7-day advance notice, and
   floors min-nights at `DEFAULT_MIN_NIGHTS` (newborn Guesty listings
   default to minNights 1). Kill `BOOKING_RULES_AUTO_PUSH_DISABLED=1`.
   Adding a new mapping-birth seam means wiring BOTH the amenities hook
   and this one — tests/booking-rules-auto-push.test.ts locks parity.
2. **`push-advance-notice-all` floors min-nights.** Saved rows from the
   2026-06-02 sweep still carry that era's `minNights: 3`; without the
   floor, re-running the sweep would push 3 back over the min-4 policy
   the rate-push piggyback enforces. Deliberate HIGHER minimums pass
   through untouched (Math.max).
3. **The Pricing tab's Booking Rules card must never silently render
   defaults as if they were live.** `GET /api/builder/booking-rules`
   returning `rules: null` now renders the amber "Never pushed to
   Guesty — showing defaults" chip. Before this, the card showed
   "7d advance notice" for never-pushed listings with no tell beyond a
   missing "Last push" line — which is exactly how the gap stayed
   invisible for six weeks.

Also load-bearing context: `GET /listings/:id/availability-settings`
404s on this Guesty tenant (only the PUT exists) — that's why
`currentBuilderBookingRulesForListing`'s availability read is
fail-soft and the push verifies advance notice from the PUT's echo.
And `calendarRules.advanceNotice.<channel>.isCutOffHoursEnabled: false`
(seen on every listing, incl. verified-protected ones) means "no
channel-specific override — the default applies", NOT "cutoff off".
...
### Preflight "Find new photos" = SAME-UNIT cross-portal photo hunt (Load-Bearing, 2026-07-17)

1. The Photo Sources card's per-unit "Find new photos" button no longer
   discovers a DIFFERENT source listing. It starts the photo-fetch job with
   `sameUnitOnly: true` (+ `currentSourceUrl` identity anchor +
   `currentFolder`), which runs `runSameUnitPhotoHunt`
   (server/same-unit-photo-hunt.ts; pure logic + tests in
   shared/same-unit-photo-hunt.ts / tests/same-unit-photo-hunt.test.ts):
   identity parsed from the saved source URL's slug
   (`parseListingAddressFromUrl` + `extractListingUnitIdentity` +
   `streetRootFromListingAddress` — the text/root parsers moved VERBATIM from
   routes.ts into shared/listing-url-address.ts; routes.ts re-imports, keep ONE
   implementation), SERP via PR #1059's `buildEquivalentPortalQueries`,
   candidates filtered to the SAME physical unit (matching normalized unit
   claim AND street root; a neighbor unit, an unprovable street, or an OTA page
   NEVER passes), each candidate's gallery scraped through the loopback
   fetch-unit-photos `{url}` mode (tiered scraper incl. sidecar), and accepted
   ONLY when dHash novelty vs the CURRENT folder proves at least
   `SAME_UNIT_PHOTO_HUNT_MIN_NEW` (3) genuinely new photos — distance >
   NEAR_DUPLICATE_DISTANCE from EVERY photo on file, hidden rows included
   (a photo the operator hid is still a photo we've seen); unhashable photos
   are inconclusive, never counted as new (do NOT borrow the theft-scanner's
   fail-toward-match posture here).
2. NO FALLBACK to different-listing discovery — that silent substitution is
   exactly what this redesign removed. An exhausted hunt fails the job with
   `recommendReplaceUnit: true` and the preflight row renders a
   "Find replacement unit" CTA (opens the page's existing UnitReplacementFlow,
   REMOUNTED via `key={targetUnit.id}` so retargeting can't commit against a
   stale unit; falls back to the manual Replace-with-URL panel when the
   property has no community folder; the CTA + its sticky receipt clear once
   the unit is actually replaced). HONESTY RULE (adversarial-review hardened):
   the flag is set ONLY when the verdict is PROVEN — the SERP sweep completed
   (`sameUnitHuntSearchComplete`: every query answered; a partial 429/timeout
   outage is NOT a complete search) AND every scraped candidate got a
   substantive verdict (`sameUnitHuntExhaustionProven`: duplicate-set /
   too-thin only — scrape-failed / no-photos / unverifiable prove nothing).
   Transient infra fails WITHOUT the flag and with retry copy, so an outage
   can never push the operator toward a destructive unit swap. The two
   permanent no-anchor states (no saved source at all vs a source URL with no
   parseable street identity) get distinct honest copy; the hunt also reads
   the folder's `_source.json` server-side when the client-sent anchor is
   missing, so a transport blip at click time can't mint a false no-anchor.
   IDENTITY PRECISION: the anchor REQUIRES a parseable street root (unit-token-
   only anchors are no-anchor — a bare unit number is how a neighboring
   building's same-numbered unit would slip through); Hwy/Highway/Loop were
   added to the shared street parsers so Hawaii highway addresses parse; the
   candidate's unit claim comes from its URL slug + parsed address ONLY
   (never raw SERP title/snippet text); Hawaii inline-unit slugs
   ("92-1070-1-Olani-St" = unit 1) carry their inline token as the unit claim
   (`hawaiiInlineUnitClaim`) so neighbors can't collapse into one identity.
3. The Unit Audit Sweep's find-new-source rung deliberately KEEPS legacy
   `findNewSource` semantics (different-listing discovery — its bedroom-
   shortfall remediation NEEDS a different listing) and never sets
   `sameUnitOnly`. Source-locked in tests/same-unit-photo-hunt.test.ts.
4. Persist stays on the single-writer paths (static: rescrape-unit-photos +
   _source.json restamp; drafts: persist-photos + the draft proof ledger), and
   galleries are NEVER unioned across portals — exactly one accepted
   candidate's gallery replaces the folder wholesale. Kill switch
   `SAME_UNIT_PHOTO_HUNT_DISABLED=1` degrades the button to the legacy
   findNewSource discovery (loudly), never to a silent no-op. Knobs:
   `SAME_UNIT_PHOTO_HUNT_MAX_CANDIDATES` (4), `SAME_UNIT_PHOTO_HUNT_MIN_NEW` (3).
...
24. **The find-new-source rung is EXACT-bedroom — representative fallbacks
   are structurally rejected (2026-07-18).** `fetch-unit-photos` has a
   "representative fallback" for ≥3BR requests: when no exact-bedroom
   gallery exists, it returns a smaller unit's gallery from the same resort
   (`representativeFallback: true`, proof status "review") — designed for
   CREATION-time flows where photos beat an empty listing. The audit
   sweep's find-new rung REPLACES a real gallery and re-stamps the unit's
   source URL/identity, so a representative result there silently turns a
   3BR unit into a 2BR one with no unit_swaps record (the Cliffs at
   Princeville draft-20 incident: weekly cron flagged a bedroom shortfall,
   find-new persisted the 2BR #3304 fractional-ownership listing as Unit
   A's source, and every later audit re-flagged the shortfall and churned
   the unit's identity again). Two layers, both required:
   (a) the photo-fetch job sends `rejectRepresentativeFallback: true` in
   findNewSource mode and the endpoint suppresses BOTH representative
   returns (best wrong-BR candidate + configured-source reuse) — the rung
   then fails honestly with "kept the existing gallery"; (b) the job
   re-checks every discovery result through the pure
   `findNewDiscoveryResultRejection` (shared/preflight-photo-discovery.ts)
   before accepting — representative flags and `bedroomMatch === false`
   never replace a real gallery, while unverifiable (null) bedroom parses
   stay acceptable. Creation-time callers (add-community wizard, empty-unit
   Find Photos) keep the representative fallback unchanged. RELATED: the
   draft repoint in PATCH /api/unit-swaps/commit/:propertyId now reconciles
   `combinedBedrooms` to the sum of unit bedrooms whenever a replacement
   changed a unit's count — before this, a 3BR→4BR replace left the stale
   combined total green-lighting the Guesty 6BR layout while the
   regenerated description honestly said "seven bedrooms" (silent
...
2026-07-19 · Jamie: "we need a button to represent that we have actually paid/bought in the attached unit … remove the buy this unit in button as the button above it is the one we now use to check the unit out … another button next to Alternative Unit that says send unit confirmation to guest" · SHIPPED (`claude/unit-buy-in-ui-updates-239c31`) · Per-slot "Mark as bought in" records the completed purchase via the existing PATCH /api/buy-ins/:id booked transition (confirm-gated + optional confirmation number; offered from awaiting_payment as "Paid — mark booked" beside "Not paid — reset" (merged with PR #1115's markBookedAfterPayment recorder — one PATCH path), from request_submitted, failed, and idle states); the "Buy this unit in" trigger for the dormant sidecar checkout job was REMOVED (scaffold kept dormant/intact — endpoints still answer); "Send unit confirmation to guest" reuses the WHOLE alternatives machinery with payload pageKind "unit-confirmation" (no relocation framing on page or message; shared/unit-confirmation-message.ts). Locked by tests/unit-confirmation-page.test.ts.
2026-07-20 · Jamie (screenshot of the alias panel): "when an email hits the units alias, automatically mark the unit as bought in — any email hitting is verification; redesign the UI of the alias emails so that it looks like a Gmail inbox with a side bar with message previews" · SHIPPED (`claude/alias-inbox-gmail-auto-bought`) · ONE marker (markBuyInBoughtInFromInboundEmail + pure shared/alias-bought-in.ts) at both IMAP ingestion seams + reconciled on both panel reads (heals pre-existing emails; responses carry autoMarkedBoughtIn/autoMarkedBuyInIds so rows refresh live); inbound-only, never re-marks booked, skips cancelled; kill ALIAS_AUTO_BOUGHT_IN_DISABLED=1. "Alias email history" is now a two-pane Gmail-style inbox (sidebar previews → reading pane, newest open by default, reply/attachments/masking preserved). Locked by tests/alias-bought-in.test.ts.
2026-07-20 · Jamie (screenshot): "The alias for both units is still the same. Please change the alias for unit B" · SUPERSEDED same day (the `unit.b.` lead-in "sounds super weird") — see the next entry; the diagnosis stands: the stored aliases were distinct but only by the trailing unit letter.
2026-07-20 · Jamie (follow-up): "don't use unit.b in the email alias. Instead have first_name.last_name.(different numbers etc). Just have the numbers at the end be different" · SHIPPED (`claude/alias-numeric-tail`) · Unit-scoped alias prefixes are now `first.last.<6-digit tail>` with the tail derived per (reservation, unit) via aliasUnitNumericTail (sha1 → 6 digits, deterministic), so sibling units get COMPLETELY different trailing numbers (thien.tran.261243 vs thien.tran.618938); reservation-level aliases keep the guest+res6 base; fallback walk unchanged. Locked in tests/simplelogin-alias-prefix.test.ts. Live: unit B re-minted again → jacelyn.tsu.410862@…; unit A's booked alias untouched.
2026-07-19 · Jamie: "When cowork attaches the buy in, attach the URL whereby clicking it takes me to the unit page with the dates already filled out. Also if possible open the link in an incognito Chrome/Safari tab" · SHIPPED (`claude/cowork-buy-in-url-prefill`) · Dated URLs: NEW pure shared/listing-stay-dates.ts (`withListingStayDates` — per-platform date-param spellings extracted VERBATIM from the find-buy-in route's inline `withStayDates`, which now delegates; NEVER clobbers params already on the URL, so a Cowork-attached URL that already carries dates is untouched); the Cowork find prompt's create-body rule now REQUIRES the attached airbnbListingUrl to embed the stay dates (concrete `?startDate=<ci>&endDate=<co>` / `?checkin=<ci>&checkout=<co>` examples per channel); and the bookings-row attached-unit "view on …" link decorates at RENDER time from the buy-in's own checkIn/checkOut — so buy-ins attached BEFORE the prompt change also open dated. Incognito: IMPOSSIBLE from a web page (browsers expose no API to open an incognito/private window — a deliberate privacy boundary; don't "fix" the copy button into a window.open). The shipped affordance is a "🕶 incognito" button beside the link that copies the DATED url + toasts the Cmd+Shift+N instruction. Locked by tests/listing-stay-dates.test.ts (31: params per platform, no-clobber/idempotence, prompt rule, routes delegation + client wiring source guards).
```
...
2026-07-15 · Follow-up to the same-day dedupe→Guesty entry above (the "separate decision if wanted" clause — now decided): the unit-audit SWEEP's dedupe hides DO sync to Guesty, at the sweep's own seam · Claude Code: the weekly cron is the "no human intervention" path, and a sweep-hidden duplicate otherwise stayed live on Guesty/OTAs indefinitely (rate pushes never touch photos; the collage pin never re-PUTs the full pictures[]) — the receipt said "N duplicate photos hidden" while the live listing kept serving them. IMPLEMENTATION (server/unit-audit-sweep.ts): the mid-sweep loopback apply body stays `{ scanId, remove }` — propertyId-free, still test-locked — because a minutes-long gallery push mid-sweep races the replace rung/collage; instead `applyRemovals` counts hides into the in-memory `dedupeHiddenThisSweep` map (jobId → count, replacedThisSweep's sibling) and `startSweepDedupeGuestySync` fires ONE fire-and-forget `repushGuestyPhotosForProperty` at sweep END, after the stage loop AND the retry rails (rail A can re-run photo-fix and hide more). SKIP RULES: a replace-rung swap this sweep already fired its own awaited re-push whose assembly drops hidden rows (the dedupe hides rode along — a second multi-minute push would be duplicate Guesty tenant load), and unmapped properties have nothing to sync; both report honestly on the photo-dedupe receipt row. Fires on completed AND failed sweeps (a later-stage error doesn't un-hide the photos), never on cancel. RATE LIMITS: self-quenching — only sweeps that hid ≥1 photo THIS run push, so the ~31-listing weekly cron pushes only where dupes were newly found (steady state: zero); sweeps run one-at-a-time (UNIT_AUDIT_CONCURRENCY) so pushes space out by sweep duration; every Guesty call rides guesty-sync's global gate. Kill `AUDIT_DEDUPE_GUESTY_PUSH=0`. KNOWN LIMIT (inherited from the Photos-tab path): a hide in a SHARED community folder re-pushes only the audited property; siblings heal on their own next push. Locked by 7 new source guards in tests/photo-dedupe.test.ts (64/0, incl. rails-before-push ordering and failed-yes/cancelled-no); unit-audit-sweep 160/0 untouched. See Photos-tab duplicate scan Load-Bearing #8 (updated).
...
2026-07-15 · Jamie: "I get the following error: TAT license lookup failed — Guesty listing compliance fetch timed out after 20s please diagnose and fix." · DIAGNOSED from live Railway logs + fixed (`claude/tat-lookup-guesty-timeout`) · NOT a lookup defect — a RATE-LIMIT PATIENCE gap: the log shows a Guesty 429 at 4:36:00 PM (the auto-reply poll hit it too), which put guesty-sync's GLOBAL gate into its retry-after pause (up to 120s — the documented posture; ALL Guesty calls serialize through it); the operator's TAT pull at 4:36:54 (listing 6a515b04…, 2381 S Kihei Rd — the Kamaole draft) queued behind that pause and its SINGLE 20s race expired → 504 → the exact toast. Same class as the 2026-07-12 amenities verify-read and the 2026-07-14 stuck-"Pushing…" (111s queued PUT that succeeded). TWO fixes: (1) PATIENCE — `fetchGuestyListingForCompliance` (routes.ts) now waits `COMPLIANCE_GUESTY_PATIENT_TIMEOUT_MS` (150s, covering a full 429 pause + drain) on its ONE queued request; deliberately NOT retried/re-issued — a timeout here means the request is still queued, and re-issuing adds queue pressure (contrast with the amenities loopbackVerifyRead, which retries a loopback HTTP call, not a gate-queued promise). `withComplianceLookupTimeout` gained a timeoutMs param; other callers keep the 20s default. (2) FAIL-OPEN — `lookupHawaiiComplianceField` (hawaii-compliance-lookup.ts) previously let a thrown Guesty read kill the WHOLE lookup even though the county-registry + public-listing legs below could still find the value; the Guesty leg is now try/caught (error remembered, loud warn) and the lookup falls through; the final no-value error appends the Guesty failure ("The Guesty listing itself could not be read: … Guesty may be rate-limited") so the 404 stays honest about the skipped source. CLIENT: `COMPLIANCE_FETCH_TIMEOUT_MS` 28s→45s (widens the window where the operator sees the value land in-tab; beyond that the previous fix's server-persist + "keeps running on the server" toast + focus re-read own the outcome). Verified: license-provenance 72/0 (4 new guards), full npm test REAL exit 0, build clean, `npm run check` 338 = baseline; post-deploy plan: re-run the operator's EXACT failing TAT pull live (listingId 6a515b04… persist=1) — expect either the real TAT from Guesty (now that the read outlasts the pause) or an honest fall-through outcome, never the bare 20s timeout.

2026-07-15 · Jamie (Pricing-tab screenshot): "Why is there still a section about static rates when we click update market rates button? Please remove that and be 100% sure that it uses SearchAPI and Airbnb to find the buy in rates." · REMOVED the panel + LOCKED the dispatch (`claude/pricing-static-rates-removal-6f3053`) · The "📊 Static rate plan" section was `StaticRatePlanPanel`, UI for the Claude static engine that the 2026-07-01 revert left DORMANT server-side — but the panel rendered UNCONDITIONALLY (the client can't read the server's `STATIC_RATE_ENGINE` env), showing every property a dead "No static rate plan yet. Click 'Update Market Rates Now' to have Claude research this resort" prompt that misdescribed what the button actually runs. VERIFIED before touching anything: `refreshMarketRatesForProperty` (routes.ts, the single dispatch for the Pricing-tab button, dashboard bulk queue, and weekly cron) gates on `staticRateEngineEnabled()` = `process.env.STATIC_RATE_ENGINE === "1"` (default OFF) → routes to `refreshHybridPricingForProperty/Draft` = the SearchAPI Airbnb MEDIAN engine — so the button was ALREADY 100% SearchAPI/Airbnb; only the panel lied. CHANGES: deleted `client/src/components/GuestyListingBuilder/StaticRatePlanPanel.tsx` + its import/render from the Pricing tab (client-only file — nothing else referenced it); the dormant server engine (static-rate-engine.ts, /api/property/:id/static-rate* endpoints, shared/static-rate-logic.ts + its 89-test suite) stays INTACT per the 2026-07-01 decision — re-enabling via STATIC_RATE_ENGINE=1 would need the panel restored from git history. LOCKED by 3 new source guards in tests/pipeline-logic.test.ts: the default-OFF opt-in check string in routes.ts, no StaticRatePlanPanel reference in the builder, and the file stays deleted. Verified: pipeline-logic green, full npm test REAL exit 0, build clean (panel strings gone from the bundle, "Update Market Rates Now" intact), `npm run check` 338 = baseline.
...
2026-07-18 · Jamie: "Check that when we push photos to Guesty it deletes all the current photos and replaces them with the ones we are pushing (60 in Guesty + 40 pushed must end at 40). In the Photos tab UI confirm the number pushed AND the number Guesty actually has. Double check it's working; improve the UI. Then merge PR with main." · VERIFIED + one real gap fixed · The replacement semantics were already correct — every `POST /api/builder/push-photos` PUT sends the complete `pictures[]`, which REPLACES the listing's gallery. THE GAP: the manual/count-mode verify accepted `savedLen >= expectedTotal`, so an eventually-consistent STALE read of the old larger gallery (60 ≥ 41) passed verification for a 40-photo push, skipped the corrective re-PUT, and over-reported `verifiedCount`. NOW: count verification requires the EXACT expected total (`savedLen === expectedTotal`) — an over-count re-PUTs and retries just like a shortfall; `verifiedCount` is capped at the pushed count (a stale over-read is an UNCONFIRMED replacement, not a bigger success); and the `done` event carries `expectedTotal` / `guestyTotal` (last read-back, null if unreadable) / `replacementConfirmed` / `staleExtra`. Strict audit mode (requiredCoverCollageUrl) is untouched. UI (Photos tab): live verify notes during the read-back ladder ("Guesty reports X, this push should leave Y…"), and a post-push confirmation panel (`photo-push-guesty-confirm`) — GREEN "Guesty gallery confirmed: N photos … All previous Guesty photos were replaced." only on an exact-match read-back; AMBER "Guesty still reports M … may not be replaced yet" (+ ↻ Re-check + Re-push guidance + destructive toast) on stale extras; AMBER dropped-photos variant on shortfall; neutral couldn't-re-read fallback. The count pill prefers the server's actual read-back, and the persisted "Last push" summary now carries the confirmed Guesty gallery count. Ledger summary format untouched in THIS PR (SUPERSEDED 2026-07-19, PR #1094 — see that Decision Log line: the summary now carries a confirmed-replacement suffix, parser extended in lockstep). LOAD-BEARING: do NOT "simplify" the count verify back to `>=` — it reintroduces the false-green stale-gallery class; source-locked in tests/guesty-photo-repush.test.ts (41/0) + client wiring in tests/push-reconcile.test.ts (64/0). WORKTREE TRAP (env, not code): `npm run check` writes `node_modules/typescript/tsbuildinfo` INSIDE a worktree, creating a stub package that shadows the real `typescript` for any later `npm test` run there — delete `node_modules/typescript` in the worktree before trusting a "Cannot find package typescript" failure.
2026-07-19 · Jamie: "When I push photos to Guesty with the manual push button in the photos tab, please have the UI confirm back to me in the timestamp how many photos were pushed to Guesty but then more importantly how many photos are now in Guesty… e.g. 40 in Guesty, pushed 33 → 7 photos removed, 33 now live in Guesty. Plan, implement, merge PR with main." · ACCEPTED + shipped (PR #1094, `claude/guesty-photo-push-confirm-4633a2`) · Extends the 2026-07-18 replacement-confirmation work (which proved the read-back but never captured the PRE-push gallery size, so "removed" was uncomputable). SERVER: `POST /api/builder/push-photos` captures `previousGuestyTotal = pics.length` off the EXISTING collage-pin pre-read (zero extra Guesty calls; null in strict-audit mode, which deliberately skips the pre-read); the done event gains `previousTotal`/`removedCount`, and `removedCount = max(0, previous − lastObservedTotal)` is minted ONLY when `replacementConfirmed` — a stale read must never claim removed/now-live counts. LEDGER FORMAT CHANGE (supersedes the 2026-07-18 "format untouched" note): `summarizePhotosPush(pushed, verified, confirm?)` appends "· N old removed — M now live in Guesty" (removed part omitted when the pre-read failed); `parsePhotosPushSummary` extended in exact lockstep returning `{pushed, verified, liveTotal, removed}` — legacy summaries parse with null replacement fields, so old ledger entries + the stream-loss reconcile keep working. When changing either, change BOTH + the round-trip locks in the same PR. UI: green confirm panel reads "Replaced the previous 40-photo gallery: 7 old photos removed, 33 now live in Guesty" (pre-read-failed fallback keeps "All previous Guesty photos were replaced."); the persisted timestamped "✓ Last push" line adds "· M now live in Guesty · N old photos removed"; the tab-strip "Pushed <time>" chip inherits the new text verbatim from the ledger. Source-locked in tests/push-reconcile.test.ts (67/0) + summary locks in tests/guesty-push-history.test.ts (77/0). Verified: full npm test exit 0, build clean (strings bundle-grepped BOTH bundles), npm run check 335 = baseline (stash A/B identical). Could NOT live-push (no Guesty creds in session) — post-deploy: push any gallery and expect the removed/now-live receipt.

2026-07-17 · Jamie: "In the pre flight check I am clicking the button find new photos. This should just try to find photos of the same unit… If it can't find different photos for that unit then the UI needs to say find replacement unit." · SHIPPED (`claude/find-new-photos-same-unit`) · The button was silently substituting a DIFFERENT unit's listing (findNewSource discovery with the current source in skipUrls) — and could even re-pick the SAME listing via another portal URL and replace the gallery with IDENTICAL photos, because nothing anywhere hash-compared a candidate gallery against the folder (fetch-unit-photos' skipUrls only suppress cross-portal same-unit clusters when the skip URL's identity parses to a root#unit key). Now the button runs a SAME-UNIT cross-portal hunt (see the Load-Bearing section above "Conventions"): anchor on the saved source URL's address+unit identity, SERP the four real-estate portals via PR #1059's buildEquivalentPortalQueries, accept only an identity-matched candidate whose gallery carries ≥3 dHash-proven NEW photos vs the current folder (NEAR_DUPLICATE_DISTANCE bar; hidden rows included; unhashable = inconclusive), persist through the existing single-writer paths (static rescrape-unit-photos / draft persist-photos). Exhausted hunts fail with recommendReplaceUnit and the row offers "Find replacement unit" (the page's existing UnitReplacementFlow); transient SERP outages deliberately do NOT set the flag. The Unit Audit Sweep's find-new-source rung keeps LEGACY findNewSource semantics untouched (test-locked). parseListingAddressFromText + streetRootFromListingAddress moved VERBATIM to shared/listing-url-address.ts (routes.ts re-imports; pipeline-logic + discovery-cache locks repointed with intent preserved). SUPERSEDES the "Find new photos = discovery for a DIFFERENT source" BUTTON semantics from the 2026-07-10 static-extension entry (the card placement + static persist rails there still stand). HARDENED by a 5-lens adversarial review (29 agents, majors all fixed): Hwy/Highway/Loop added to the shared street parsers (the 57-091-vs-57-101 Kamehameha Hwy neighbor class could otherwise pass on a bare unit token), unit-token-only anchors rejected, Hawaii inline-unit slugs (92-1070-1-Olani-St) carry their token as the unit claim, candidate unit claims never read raw SERP snippets, recommendReplaceUnit requires a COMPLETE SERP sweep + every candidate substantively judged (scrape-failed/no-photos/unverifiable prove nothing → retry copy, no flag), the hunt reads _source.json server-side when the client anchor is missing, UnitReplacementFlow remounts on retarget (key), and the CTA/receipt clear once the unit is replaced. Verified: same-unit-photo-hunt suite green (incl. the neighbor/inline/honesty locks), full npm test REAL exit 0, build clean, npm run check per-file error sets identical to base (A/B'd — 0 new; the repo-wide count moved off the documented 338 with main, not with this change).
...
2026-07-17 · Jamie: "Please just double check that when I click produce the descriptions button in the descriptions tab that the AI looks at the photos, and what is confirmed in the bedding tab etc and that it is an accurate description. If not, please fix it so that it is and merge PR with main." · VERIFIED-NO then FIXED (`claude/descriptions-button-ai-check-a34972`) · The suspicion was correct: "↻ Regenerate descriptions" sent only bedrooms + `_source.json` URL + address; the endpoint's `unit1.description` source-fact leg was DEAD CODE (no caller ever populated it), so the prompt literally asked Claude to ESTIMATE bedding/bathrooms and nothing from the photos or Bedding tab ever reached the generator. FIX (three grounding sources, all fail-soft): (1) `/api/community/generate-listing` accepts `propertyId` and grounds the prompt SERVER-side in the per-photo Claude-vision captions (via `buildPhotoCommunityCheckRequestForProperty` — active swap folders, hidden filtering, and drafts' negative ids inherited; unit groups matched by their deterministic "Unit A/B" labels, never array position, because a photo-less unit is simply absent) plus the saved Amenities-tab set (`getPropertyAmenities` → `getAmenityLabel`); the builder button and the audit sweep's regenerate twin both pass `propertyId`. (2) The builder button additionally sends each unit's CONFIRMED Bedding-tab config (`describeUnitBedding`, matched by canonical `unitId` with index fallback) — CLIENT-ONLY by necessity (localStorage, #9); the prompt marks it AUTHORITATIVE (outranks source listing details + photo evidence), and the response's structured `bedding` field is deterministically OVERWRITTEN with it (`withConfirmedBedding`). (3) ACCURACY IS CHECKED, NOT TRUSTED: pure `unconfirmedBedTypeMentions` (shared/description-copy.ts — "bed"/"bedroom"-anchored patterns so "a single bedroom unit"/"full kitchen" can't false-positive; "queen sleeper sofa" reads as the sofa bed) audits summary/space (only when EVERY unit sent a confirmed config) + per-unit short/longDescription; a mismatch triggers ONE corrective retry, and a persistent mismatch surfaces as `accuracyNotes` — deliberately NOT `warning`, which every automated consumer refuses wholesale — rendered as a review toast in the tab. A 3-lens adversarial review confirmed and fixed FIVE defects in the first cut (all regression-locked): (a) the confirmed regexes ran over the WHOLE describeUnitBedding string, so "Double vanity"/"Full Bath" bathroom text masked invented double/full beds → the audit now cuts each unit's string at its own "Bathrooms:" section (array form — join AFTER cutting, or unit 2's beds vanish behind unit 1's bathrooms); (b) plural labels ("2 Kings"/"2 Queens"/"2 sofa beds") failed \bking\b-style matching → guaranteed false flag + self-contradictory retry → confirmed regexes are plural-tolerant; (c) an audit-clean but structurally degenerate retry (dropped unitA / gutted longDescription) could replace a complete first draft with fallback filler → retries are gated on `generatedDraftCompletenessRegressions` = []; (d) the model's `bedding` field was audited though deterministically overwritten → excluded; (e) draft configs carry bed types outside GuestyBedType (TWIN_BED/FULL_BED/CAL_KING_BED from draft-bedding.ts) which rendered "undefined" in the authoritative string → describeUnitBedding humanizes unknown enum names. LOAD-BEARING: keep `accuracyNotes` out of `warning`; keep the confirmed-bedding CONTEXT lines label-stable (test-locked count 3 across both prompt variants); the grounding gather runs AFTER the no-key early return so the fallback path stays byte-identical. Locked by tests/description-copy.test.ts (78/0) + tests/bedding-space-copy.test.ts (describeUnitBedding humanize); tests/pipeline-logic.test.ts's unitA-coercion lock repointed to the wrapped construction (intent preserved).

2026-07-18 · Jamie: "Now that we are using the clubhouse address as the published address for all listings we no longer need to see if someone is listing our units address. Therefore please remove the detection system for if a unit has the same address as ours. The only detection system that now matters is the photo detection system. … remove the address check/found alerts and icons from the dashboard and anywhere else that this information is being displayed. Then merge PR with main." · ACCEPTED — the address-on-OTA detection leg (2026-06-29 → 2026-07-12 phases) is REMOVED wholesale; the photo reverse-image leg is the ONLY detection system · REMOVED: the scanner's address leg (per-platform site: SERPs, deep-fetch recall, coordinate cross-check, geocode street fallback, address-only backfill + POST /api/photo-listing-check/address-backfill), the 4 photo_listing_checks address columns (schema-maintenance now DROPs them idempotently on boot — required, because drizzle-kit push aborts non-interactively on a destructive diff and would stay dirty forever; do NOT re-ADD them), shared/address-listing-logic.ts + address-page-match.ts + address-geo-match.ts + address-alert-warning.ts (+ their 4 test suites), the dashboard 📍 A/V/B mini-row + "Addr on …" line + orange address-alert banner/popup + scan-modal address dots/legend, the address-found operator SMS (photo-found SMS unchanged), the audit sweep's address verdicts (stage-ota and the photo-fix OTA trigger are photo-only now), and every PHOTO_LISTING_ADDRESS_* env knob. KEPT (load-bearing): parseStreetCityState moved VERBATIM to new shared/address-parse.ts (tests/address-parse.test.ts) — server/published-address.ts, server/unit-audit-sweep.ts, and server/auto-replace-jobs.ts import it for TARGET street/city resolution, unrelated to detection; and the CANDIDATE-side OTA screens (server/combo-ota-preflight.ts, routes' runOtaQualifier) keep their OWN address searches — they qualify OTHER people's units as photo-sourcing candidates, a different system. decidePlatformStatus lost its vestigial hasAddressHit input. Old photo_listing_checks rows may carry a legacy "Address search unavailable" errorMessage — photoListingScanWasInconclusive deliberately ignores it (test-locked). This supersedes the address halves of the 2026-06-29 detection entries and the 2026-06-30 address-inconclusive + 2026-07-06 grey-📍 entries (marked inline).

2026-07-18 · Jamie: "Please investigate and make sure that the scan photos for duplicates button in the photos tab of the unit builder scans all the photos for both units and checks if there are too many angles of the same shot etc. Then update the methodology, if this does need to be improved and merge PR with main." · INVESTIGATED + IMPROVED (`claude/unit-builder-duplicate-scan-3dbfa5`) · BOTH UNITS: already correct and now PROVEN in the UI — the request is built from the rendered `photos` array, which builder.tsx assembles as every unit gallery in unit order followed by the community folder, so all three folders were always sent as separate groups. The gap was that folders with no findings were FILTERED OUT of the result (`folders.filter((f) => f.groups.length > 0)`), so "Unit B was scanned and is clean" and "Unit B silently never reached the scan" rendered identically; a per-folder coverage receipt now renders for every scanned gallery. ANGLES OF THE SAME SHOT: three real suppressions found, all fixed. (1) COVERAGE — the manual path planned ONE evenly-strided sample capped at PHOTO_DEDUPE_VISION_CAP=60 (`evenSampleIndices`), so a 150-photo gallery showed Claude 60 photos and never looked at the other 90, while `visionComplete:false` was computed, sent over the wire, and consumed by nobody; since dHash structurally cannot catch two angles of one scene (they sit far above NEAR_DUPLICATE_DISTANCE), those 90 photos had NO angle check at all and the tab still said "✓ No duplicate or redundant photos found across 150 photos". Replaced with tiered `buildManualVisionBatchPlan` (one call → exhaustive pair-cover → disjoint chunks that at minimum show every photo once) plus a scan-wide `PHOTO_DEDUPE_SCAN_BUDGET_MS` (8 min) guarding the synchronous response against Railway's 15-min edge cut; the local sampler is deleted and test-locked against reintroduction. (2) STAR TOPOLOGY — vision groups unioned only `valid[0]`↔`valid[i]`, so one edge blocked by the category/bedroom guard discarded every OTHER pair in the group; now gated per pair, so a partially-guarded group degrades to its allowed sub-clusters instead of collapsing to nothing. (3) DOUBLE CONSERVATISM — medium-confidence groups were discarded outright ON TOP of a prompt that already says "when unsure, DO NOT group", and a guard-blocked high-confidence edge vanished with no trace even though an angle variant can NEVER be rescued by the exact-hash bypass. Both now surface as a REVIEW tier (`DedupeFolderResult.reviewGroups`, kind "review"): reported with thumbnails, nothing pre-selected, never auto-fixable. KEPT DELIBERATELY SEPARATE from `.groups` because the sweep reads `folders[].groups` emptiness as "gallery is clean" — routing review candidates there would make every weekly cron flag properties over findings it can never act on; three independent barriers stop a sweep auto-hiding one (positive kind partition, the new explicit `kind === "review"` skip in dedupeAutoFixSelections, and `keep:true` on every review member). `validateDedupeSelection` accepts review members so a deliberate operator click still works, and now counts DISTINCT filenames per folder since one photo may appear in both tiers. HONESTY: `summarizeDedupeFolders` warns on `!visionComplete` (wording splits the signals — the hash pass genuinely covered everything), and the tab renders warnings + note on EVERY completed scan; they used to live inside the `groupCount > 0` branch, i.e. hidden on exactly the branch where a false clean is dangerous — a folder whose AI pass errored showed a bare green pass. Prompt now names the target ("TOO MANY ANGLES OF THE SAME SHOT") and tells the model captions are machine-generated, so the vision leg is a genuine check on the labeler rather than an echo of it. Scope note: grouping stays WITHIN-FOLDER (cross-folder duplicates remain the photo-community-check's signal) and the scan still only PROPOSES. HARDENED BY ADVERSARIAL REVIEW (4 lenses, 19 confirmed findings, all fixed pre-merge) — the majors were mine to begin with: (a) the new per-pair gating let an UNLABELED bridge photo transitively re-merge two different bedrooms into a confirmed, sweep-auto-hideable group (guards are now enforced cluster-wide, regression-locked); (b) the HTTP-shaped scan deadline was defaulted inside the engine, so NON-fullAutomation sweeps silently inherited it — the budget now belongs to the route; (c) the budget was consumed in folder order, so the LAST gallery (typically Unit B) could get zero angle coverage — each folder now gets a fair share of the remaining time; (d) "hash-only results shown" rendered while AI groups were on screen after a partial pass, and its else-if suppressed the coverage warning; (e) the client keep-one guard checked only the clicked group, so an operator could build a selection the server 422s (a filename can sit in both tiers); plus the tier-3 orphan fold emitting a batch of cap+1, a covered[] that claimed an unsent photo, a contradictory "⚠ saw 150 of 150" receipt line, in-progress copy hardcoding gallery names it cannot know, and a comment of mine asserting the exact opposite of what the code does. Verified: photo-dedupe 122/0 (was 67/0), unit-audit-sweep 200/0 untouched, full `npm test` exit 0, build clean, `npm run check` 335 = baseline (stash A/B, identical per-file error sets). Could NOT live-run the Claude vision leg (no ANTHROPIC key in session) — post-deploy: open a Photos tab and click "🧹 Scan photos & remove duplicates"; expect a per-gallery coverage receipt naming both units and the community folder, and an amber "Possible repeat shots — your call" panel where the AI was unsure. See Load-Bearing "Photos-tab duplicate scan" points 3-5.

2026-07-18 · Jamie (report screenshot: Wavecrest Unit A/B "Page unreadable" on the Source Page Check): "Please diagnose why it says page unreadable. Also, if it needs to scrape a page please utilize the apify scraper services instead of the local sidecar, unless you feel that the sidecar is genuinely better." · DIAGNOSED + SHIPPED (`claude/page-unreadable-error-cb62ce`) · ROOT CAUSE (don't re-chase): the source-page community leg's `fetchSourcePageHtml` was a bare node `fetch()` with zero scraping tiers — Zillow 403-PerimeterX-walls EVERY plain HTTP fetch (live-verified: HTTP 403 even from a residential IP with a browser UA; from Railway's datacenter it is guaranteed), so every Zillow-sourced unit rendered the honest-but-useless "Page unreadable" and the leg's provenance/gate value was lost for exactly the portal most source URLs live on. FIX: an APIFY RESCUE TIER in `server/source-page-community-check.ts` — when the direct fetch fails OR returns a bot-wall page (new pure `looksLikeBotWallPage`, title markers always / body markers only on compact pages so a real listing mentioning "cloudflare" in a script bundle never false-positives), the check runs the SAME Apify detail actor the photo pipeline already pays for (zillow→APIFY_ZILLOW_ACTOR, redfin→APIFY_REDFIN_ACTOR, realtor→APIFY_REALTOR_ACTOR; other hosts stay direct-fetch-only) and builds the location signals from its STRUCTURED JSON via new pure `signalsFromListingJson` — full street/city/state/zip + listing description, i.e. richer signals than scraped HTML. LOAD-BEARING in that extractor: side-panel subtrees (similarHomes/nearbyHomes/comparableHomes/schools/...) AND agent/broker subtrees (attributionInfo/listingAgent/office/...) are SKIPPED — a nearby-homes card or an agent's office city names a DIFFERENT place and would poison the verdict toward a false "no"; and bare `name` is NOT a title key (live smoke: the listing AGENT's name won the title slot until excluded). Per-URL in-memory cache (12h success / 10min failure, cap 300) because the audit sweep's consensus rail re-runs the full check up to 3× — never triple-spend Apify on the same URL. Verdicts read via the rescue carry "(page read via Apify scraper)" in the reason (report-UI honesty). Deliberately NOT the local Chrome sidecar: this is a text/address check, not a gallery harvest — Apify's structured JSON is strictly better here and doesn't depend on the operator's Mac or the sidecar wallet; the sidecar was never in this path. Fail-soft posture unchanged: no token / unsupported host / actor failure / kill switch `SOURCE_PAGE_APIFY_RESCUE=0` all keep today's "unreadable → uncertain, never a false no". Timeout `SOURCE_PAGE_APIFY_TIMEOUT_MS` (150s). Live-smoked end-to-end pre-merge: real Zillow URL → direct fetch BLOCKED → Apify 5s/6 address hints → Claude "yes" 0.97 naming the expected community. Locked by tests/source-page-community.test.ts (66/0, incl. source guards on the rescue default, the bot-wall screen, the host map, and the kill switch).
  FOLLOW-UP (same day, second PR `claude/source-page-parse-retry`): the post-deploy smoke on the REAL Wavecrest units surfaced an intermittent "Source-page analysis unavailable (Could not parse JSON)" on Unit A. NOT truncation — the model sometimes emits a JSON verdict, RECONSIDERS ALOUD, then emits a corrected second JSON (captured verbatim in the raw response), and parseJsonLoose's first-brace-to-last-brace fallback spans both objects + prose and fails. TWO fixes: (1) pure `extractLastJsonObject` (string-aware brace scan; the LAST balanced object = the model's final answer) salvages the raw text before a bounded one-retry; (2) the ROOT trigger was prompt confusion — the model read our internal slot label "Unit A (2BR)" as a unit-NUMBER claim and answered "no" (0.97!) because the page says "Unit B310", community match notwithstanding; the prompt now declares the label an internal slot name, NOT a unit-number claim (test-locked "internal slot label"). maxTokens 400→600 for headroom. Behaviorally verified: the exact failing unit went 4/4 clean "yes" 0.99 post-fix (was ~50% parse-fail). tests/source-page-community.test.ts now 75/0.

2026-07-18 · Jamie (preflight screenshot: Wavecrest "Find a New Unit" — "No eligible replacement units found. Google returned 213 listing link(s) for '8001 Kamehameha V Hwy' / 'Wavecrest Resort', but all 182 were filtered out because they did not match the resort street"): "What can we do to allow the pre flight check tool to find a replacement unit for this listing? Please then implement that methodology/fixes so that future unit replacement searches for this listing and others will work." · DIAGNOSED + SHIPPED (`claude/preflight-unit-replacement-749b1d`) · ROOT CAUSE (don't re-chase): the draft's community address "8001 Kamehameha V Hwy" is a directory/office address — every real Wavecrest unit is indexed on Zillow/Redfin/Realtor under the BUILDING addresses 7142/7144/7146/7148 Kamehameha V Hwy, Kaunakakai (web-verified: Zillow's own /b/ building pages are "Wavecrest - 7142…" and "Wavecrest - 7146…"). The find-unit resort-street gate requires an exact root match for plain-numbered streets (the Hawaii district-lot family tolerance in shared/hawaii-street-family.ts only applies to "69-180"-style pairs), so ALL 182 real listings were rejected → total strikeout. TWO-LAYER FIX: (1) curated `CommunityAddressRule` for Wavecrest (names Wavecrest/Wavecrest Resort/Wavecrest Oceanfront Resort; street 7142 Kamehameha V Hwy; city Kaunakakai + Molokai/East Molokai/Ualapue/Pukoo aliases; `buildingStreetRoots` 7142/7144/7146/7148; `zillowBuildingUrl` = the trusted 7142 /b/ page) — the gate unions the rule roots with the draft's stale 8001 so the search works WITHOUT touching the draft row, and `validateCommunityStreetAddress`/`resolveBulkComboListingStreet` now steer the draft's street to 7142 on the next save/refresh (the Ko Olina Hillside rule-first pattern). (2) GENERAL name-anchored sibling street-root RESCUE (`shared/discovery-root-rescue.ts` + routes.ts wiring) so ANY community with a wrong configured street NUMBER self-heals: on a total gate strikeout, roots are learned from rejected results that name the community as a contiguous phrase, share the configured street name+type (number-only difference), and recur across >=2 distinct listings; lot-significant streets (Waikoloa Beach Dr class) excluded wholesale; rejects replayed through the widened gate BEFORE the second-wave root queries so those + the Apify/RealtyAPI legs inherit the learned roots; the "all filtered out" diagnostic now says whether the rescue found sibling-street evidence or the street itself looks wrong. See Load-Bearing "Find-unit sibling street-root RESCUE". Verified: tests/discovery-root-rescue.test.ts 29/0 (in the npm chain; incl. Colony-Villas token-scatter anchor rejection + wiring source guards), community-addresses 66/0, full `npm test` REAL exit 0, build clean (rescue log + 7142/7146 strings bundle-grepped), `npm run check` 335 = baseline (stash A/B; the one new TS2802 fixed via Array.from). Could NOT live-run the SearchAPI leg (no key in session) — post-deploy: re-run "Find replacement unit" on the Wavecrest preflight; expect Railway "[find-unit] Community address roots:" to list the 7142/7144/7146/7148 roots and candidates to survive discovery.

2026-07-18 · Jamie (screenshots: builder Descriptions/Photos + the Redfin source page): "The first unit is actually a 2 bedroom, not a 3 bedroom … fix the photos page so it accurately shows 2 bedrooms … make sure the description matches … I think this was originally 2 3 bedroom units but when we replaced units it replaced it with a 2 bedroom unit and a 4 bedroom unit. Please diagnose … so this doesn't happen in the future … and fix this unit. Then merge PR with main." · DIAGNOSED LIVE (prod DB + `replacement_find_jobs.v1` + Railway deployment logs at the exact 03:01Z window) + FIXED (`claude/unit-bedroom-count-fix-c2f399`) · The Cliffs at Princeville (draft 20) was created 2026-06-16 as 3BR+3BR (units #2103/#4204). The corruption came from TWO different mechanisms: (1) UNIT B, 2026-07-12 audit sweep: the photo-fix ladder legitimately replaced it (find-unit's bedroom gate is a MINIMUM, so a 4BR passed a 3BR requirement; swap 64 → Unit #9203, repoint set unit2Bedrooms=4) — but the repoint never touched combinedBedrooms, so 3+4=7 drifted against the recorded/Guesty 6 and the regenerated description said "seven bedrooms" under a "6BR" title with the audit layout stage validating the stale 6. (2) UNIT A, 2026-07-18 weekly cron (the smoking gun): photo-community flagged "bedroom photos 0/3", the find-new-source rung requested an exact-3BR discovery, fetch-unit-photos correctly SKIPPED every 2BR candidate ("2BR does not match requested 3BR" — logged) and then returned its REPRESENTATIVE FALLBACK anyway (the 2BR #3304 fractional-timeshare listing, proof status "review"); the job's acceptance check only rejected status "rejected", so the wrong-bedroom gallery REPLACED unit A's real gallery + source URL with no unit_swaps record, and persist-photos re-stamped a clean "accepted" proof that ERASED the representative/bedroom-mismatch evidence. The subsequent replace rung then failed ("2 had too few bedrooms") because the resort genuinely has no clean 3BR for sale — leaving a 3BR-configured unit permanently backed by a 2BR listing, re-flagged and re-churned by every weekly audit. FIX (see Load-Bearing Unit Audit Sweep #24): find-new mode sends `rejectRepresentativeFallback` (endpoint suppresses both representative returns) + the pure `findNewDiscoveryResultRejection` acceptance guard; the draft repoint reconciles combinedBedrooms. LIVE FIX: operator chose to accept reality — unit1Bedrooms 3→2 (2+4=6 matches the Guesty 6BR layout), bedding/short-description columns updated, descriptions regenerated via the live generate-listing→overrides→push chain (Claude, grounded in the real 2BR/4BR sources; summary/space now say "Unit A (2BR)"/"Unit B (4BR)", sleeps 16) and push-descriptions verified on Guesty. Verified: tests/find-new-bedroom-guard.test.ts 13/0, full `npm test` REAL exit 0, build clean, `npm run check` 335 = baseline (git-stash A/B, identical per-file error sets).

2026-07-18 · Jamie: "Make every listing and all future listings created via the add combo listing feature, have a minimum nightly stay of 4 nights. At the moment this is set to 5 nights. Please update all the listings in the system and push to Guesty." · ACCEPTED + shipped (`claude/combo-listing-min-stay-4-351359`) · The standing nightly minimum is now **4** — `DEFAULT_MIN_NIGHTS` 5→4 (server/routes.ts, still the single source of truth) plus the four creation-path defaults the 2026-07-09 entry enumerates: `builder_booking_rules.min_nights` column default (shared/schema.ts), the client `DEFAULT_BOOKING_RULES` (GuestyListingBuilder), the full-build publish default (builder.tsx), and the `guestyService.updateBookingSettings` fallback. Add-combo drafts inherit it through the same path they always did (creation defaults + the per-push floor once mapped+priced) — no combo-specific code was needed. **LOAD-BEARING — a policy DECREASE does not travel through a FLOOR.** `enforceListingMinNights` skips any listing already at or above the minimum, so flipping the constant alone would have left all ~24 mapped listings sitting at 5 forever while every surface claimed 4. It now takes a `mode`: **"floor"** (unchanged, and still the per-push default on `push-seasonal-rates`) only ever RAISES; **"policy"** (used by `POST /api/builder/booking-rules/push-min-nights-all` only) also LOWERS a listing at or below the retired policy value down to 4. New constant `LEGACY_POLICY_MIN_NIGHTS = 5` draws that line: a minimum **strictly above 5 is treated as a deliberate operator override and is preserved even in policy mode** — this is what protects Menehune Shores' 7-night loss-prevention rule (Decision Log 2026-06-27), which a blanket set-to-4 would have silently reverted back into the documented book-at-a-loss state. Policy mode is deliberately NOT the per-push default: a routine rate push must never walk a future operator-set 5 down to 4 behind the operator's back — rolling a policy change out is an explicit, operator-triggered act. Everything the 2026-07-09 entry marked load-bearing is intact: confident-read-only (an empty/failed terms read is still a no-op), only `terms.minNights` is written (maxNights / cancellationPolicy / instantBooking carried back, availability-settings never touched), still strictly non-fatal on the rate-push path, and the "do NOT simplify this back to `pushBuilderBookingRulesToGuesty`" warning still applies. Verified: full `npm test` exit 0, `npm run build` clean, `npm run check` 335 = baseline (git-stash A/B, identical per-file error sets). Live rollout ran post-merge via `push-min-nights-all` — see the PR body for the per-listing result.
...
2026-07-19 · Jamie (screenshot of the "Auto Cowork — find and attach the cheapest buy-in units" modal sitting over the bookings page with Safari's "Do you want to allow this website to open Claude?" confirm on top): "I don't want the prompt to pop up like that. It just needs to push into Claude Cowork. Also, when it pushes into Claude Cowork please ensure that it starts the task automatically. Then when it has attach the buy ins please ensure the claude cowork button to check out is there and automatically pushes into claude cowork when I click the button." · ACCEPTED for asks 1 and 3; ask 2 (auto-start) REFUSED AS IMPOSSIBLE with evidence · SHIPPED (`claude/cowork-prompt-flow-1a70b5`): (1) NO MODAL ON THE HAPPY PATH — all six Cowork buttons (find, checkout, verify-community, guest-happy, find-on-VRBO, bulk) drop their per-component `open`/`copy`/`<Dialog>` and share a new `useCoworkLaunch` hook (re-entrancy guard + `disabled` while launching). ONE page-level paste-fallback dialog remains, gated by the new pure `coworkLaunchNeedsFallback()` (`!launched || !promptIncluded`, `abortedForAuth` suppresses it) — it opens only when the brief genuinely did not reach the composer. Page-level ON PURPOSE: the row dialogs sat inside poll-driven conditional wrappers, so a dialog opened after an `await` could be unmounted before it rendered, on exactly the path where it holds the operator's only copy. (2) AUTO-START IS NOT POSSIBLE — see the new paragraph under "Cowork browser checkout is a human-payment handoff". Claude Desktop 1.22209.3's handler forwards only `q`/`folder`/`file` to the remote claude.ai composer; the cloud dispatch `startSession` channel that CAN auto-start is account-scoped and unreachable from a web page; an osascript keystroke via the sidecar was rejected on blast radius. Nothing was built; the operator is told plainly that one Enter press remains, and that it is the press that authorises spending. (3) ASK 3 WAS A DATA-FRESHNESS BUG, not a render-condition bug — the checkout render guard was already correct. Cowork attaches OUT OF BAND (no job id, no server handle, nothing to poll) and the bookings queries skip hidden refetch ticks, so a returning operator faced a button-less row. Added NEW pure `shared/buy-in-slot-signature.ts` + `POST /api/operations/buy-in-slot-status` (ONE `buy_ins` query, ZERO Guesty) probed on `window.focus` AND `visibilitychange` (BOTH — TanStack's focusManager listens to visibilitychange only, which never fires in the browser-beside-Claude-Desktop layout the whole feature assumes) plus a 15s poll while a launch-armed window is open. It compares the server against WHAT IS ON SCREEN, not against the previous probe — probe-to-probe would seed and silently miss an attach that landed before the tab's first wake event. ALSO FIXED, found while verifying and the reason the operator's own screenshot property was one rename away from breaking: the find brief measures 14,298 chars for "Menehune Shores - 4BR Condos - Sleeps 12" against the 14,336 cap and 14,483-14,504 for longer names — OVER cap `buildCoworkDeepLink` correctly refuses to embed, so Cowork was opening with an EMPTY composer. `launchCoworkPrompt` is now CAP-FIRST (direct embed, relay only on overflow, new `cowork_prompt_runs` kind `"find"`), which also makes the checkout brief embed directly so the money prompt is readable in the composer instead of a ~400-char bootstrap. THREE defects in my own first cut were caught by adversarial review and fixed pre-merge, all invisible to tests: the in-flight-fetch starvation guard used `getQueryState` (an EXACT queryHash lookup) against bare prefixes on parameterized keys, so it was dead code — now `isFetching({queryKey})`, empirically verified against the installed query-core; the checkout freshness pre-flight compared two DIFFERENT populations (rendered slots drop a buy-in whose unitId isn't a configured slot; the probe returns every row), so one orphan would make the signatures never converge and permanently brick the money button behind "click again" — it now compares only the buy-ins the click will send; and the pre-flight failed open SILENTLY under the normal success toast, which would let unverified prices arm the 15% guard invisibly. Verified: cowork-launch 59/0, NEW cowork-refresh 30/0, cowork-buyin-prompt 284/0, full `npm test` REAL exit 0, build clean, `npm run check` 335 = baseline (git-stash A/B), and the three asks proven on the BUILT bundle (static SPA server + Playwright, mocked /api — 11/11: click opens no dialog, a `claude://cowork/new?q=` link fires with the prompt actually inside it, the checkout button appears after a simulated out-of-band attach with no manual refresh, and clicking it pushes straight through with the full brief embedded).

2026-07-19 · Jamie (Claude Desktop screenshot of a Cowork task titled "Rental portal deep-link test" that ran a "Deep-link plumbing test… / LENGTH TEST… inert filler" prompt and replied "deep link OK"): "See the attached error from Cowork. Please fix and implement so that it no longer runs as a test. Then merge PR with main." · DIAGNOSED — NOT fired by the product (don't re-chase) · The portal has NEVER shipped a test prompt: the strings appear in NO commit in git history, and a live grep of the deployed bundle (`/assets/index-Bqpy6CEt.js`, deployment 59015dbd post-#1096, fetched with the auth-excluded assets path + ADMIN_SECRET via `railway run`) shows 0 hits for "plumbing test"/"LENGTH TEST"/"inert filler" while the real `claude://cowork/new` launch path IS present. ROOT CAUSE: the 2026-07-13 Auto Cowork session live-fired two ad-hoc `open "claude://cowork/new?q=…"` test links on the operator's Mac to verify the deep-link contract (transcript-confirmed, 12:52Z + 12:56Z); nobody pressed send, the SECOND link's text CONCATENATED into the same unsent composer (both tests visible in one message bubble — deep links appended to an unsent draft merge, they don't always mint a fresh task), and the stale task sat in Cowork until the operator sent it on 07-19 — producing the "portal is running tests" impression. FIX SHIPPED (`claude/cowork-error-fix-7ed99b`): (1) drift-lock in tests/cowork-launch.test.ts — walks ALL shipped source (client/src, server, shared, daemon, scripts; tests/ + AGENTS.md excluded since they legitimately NAME the markers) and fails on any dev-test prompt marker (plumbing test / LENGTH TEST / inert filler / BROWSER CLICK TEST) or any hard-coded prompt after `cowork/new?q=` (only `<placeholder>`/`${template}` forms allowed — buildCoworkDeepLink's encodeURIComponent is the sole sanctioned prompt carrier); negative-tested (a planted marker fails the suite with the file named). (2) New Load-Bearing paragraph "NEVER live-fire test prompts into the operator's Cowork" under the Auto Cowork section — dev sessions verify deep-link plumbing via the unit suite + bundle greps + reading the app's handler, never by firing claude:// links with test content; unavoidable live fires need explicit operator approval + same-session task deletion. OPERATOR ACTION: delete/archive the "Rental portal deep-link test" task in Cowork — it is inert leftover, not a portal artifact. Verified: cowork-launch 60/0 (was 59), full `npm test` REAL exit 0, build clean, `npm run check` = baseline.

2026-07-19 · Jamie (after the deep-link-test cleanup, asking whether the Cowork prompt can push AND start automatically): "Is there anyway to automate starting the task…?" → options laid out (headless CLI runner / dormant BUYIN_ENGINE / keystroke hack) → "Hmm, I like number two, explain this more?" → "I like it. If there's no cowork window, how do I see like the cowork feedback/comments?" → portal-native run-log design → "Build this all out and then merge PR with main." · ACCEPTED — HEADLESS CLAUDE FIND-RUNS (`claude/cowork-error-fix-7ed99b`, second PR) · The find flow gains a zero-click twin: "Auto-run · find cheapest (no window)" POSTs the SAME CoworkBuyInPromptInput to the new `POST /api/claude-find-runs`; the Mac daemon's runner child (claude-find-runner.mjs, spawned by worker.mjs LOCAL slot 1 only) claims it (X-Admin-Secret), ensures a DEDICATED hidden persistent-profile Chrome (CDP 9250 — cookie clearances accumulate; never the personal Chrome or the 8 scrape instances), and runs the brief via `claude -p --output-format stream-json` with the locked allowlist (mcp__chrome via chrome-devtools-mcp, curl-only Bash, WebSearch/WebFetch). The brief is buildCoworkBuyInPrompt + NEW `headlessRun` opt (default byte-identical, bulkBrief-style lock): attach calls point at run-scoped token proxies `/api/claude-find-runs/agent/:id/{buy-ins,attach}` (auth-excluded prefix + timing-safe in-handler token check; proxies pin propertyId/reservation/dates, reject airbnb.* hosts, forward via loopback), bot walls become `ATTENTION:`/`RESUMED:` marker lines (wrapper chimes Sosumi + portal amber state), and the agent's FINAL message is the persisted report. Feedback is PORTAL-NATIVE (the operator's ask #4): HeadlessFindRunPanel on the bookings row renders live narration/actions, attention, error, and the report — server-persisted (`claude_find_runs.v1`, promise-tail, bounded 250-event ring, client view STRIPS token+prompt), so it reads from a phone; the panel mounts OUTSIDE the empty-slots box because attaches make that box disappear exactly when the report matters. Watchdog closes orphans honestly (unclaimed 5m = "Mac runner never picked this up", silent 10m, 90-min ceiling). FIND-ONLY IS LOAD-BEARING: checkout remains the human-gated Cowork prompt (send = money approval; payment human-only). VERIFIED ON THE MAC before merge: headless `claude -p` mechanics + stream shape proven live; CLI login is SEPARATE from Claude Desktop (clean-env run says "Not logged in") so runs fail fast with the one-time setup instruction (`claude` → /login, or CLAUDE_FIND_RUN_API_KEY); full offline e2e smoke (mock portal + REAL runner + REAL claude spawn): claim → Chrome up → 6 token-scrubbed flushes → exact auth-failure terminal. Tests: NEW tests/claude-find-run.test.ts 83/0 in the npm chain (store/single-flight/watchdog/view-stripping, brief variant incl. byte-identity + no-sounds + marker protocol, runner .mjs classification + TS-twin EQUIVALENCE lock — two implementations exist because the daemon runs plain node — plus source guards on allowlist/slot-gate/auth-prefix/pinning); cowork-buyin-prompt lock repointed 2→3 for the remaining-budget rule (headless button is a third consumer). Full `npm test` REAL exit 0 (PIPESTATUS-checked), build clean (labels bundle-grepped BOTH bundles), `npm run check` 335 = baseline. Post-merge: runner file cp'd to the LIVE ~/.vrbo-sidecar-daemon + worker.mjs hook applied surgically (live copy is AHEAD of main) + kickstart. OPERATOR ONE-TIME SETUP before first real run: `claude` in Terminal → `/login`.

2026-07-19 (second PR) · Jamie, after live-testing the first: "It's not actually starting the task in cowork though. I have to manually click the start button. This is a concern for the bulk queue because it will require my human input which defeats the purpose of the queue. How can we fix this?" · DIAGNOSIS REFRAMED THE ASK, then ACCEPTED · The send press was the SMALLEST part of the problem and fixing it was impossible anyway (see the auto-start paragraph under "Cowork browser checkout is a human-payment handoff"). Measured cost of a bulk batch of 8 BEFORE this change: ONE send press (the batch is a single Cowork task, `buildCoworkBulkFindAndPreparePrompt(inputs)` called once), but ~16 card entries and ~16 "I clicked Checkout" confirmations, strictly serialized by the combined brief's "Exactly one outstanding payment handoff at a time". So the queue could never be walked away from no matter what happened to the send press. SHIPPED (`claude/cowork-prompt-flow-1a70b5`, PRs #1096 + this one) in three phases. P1 — BULK IS FIND-ONLY: `startBulkCoworkFind` now calls `buildCoworkBulkBuyInPrompt` (which already existed, fully tested, and ends every brief at ATTACH) with relay kind `"bulk-find"`; one send, then walk away. `buildCoworkBulkFindAndPreparePrompt` is retained and still tested but is DORMANT — deliberately not deleted, because this operator has reversed bulk routing twice (2026-07-13, 2026-07-19) — and a source guard now fails if the client is ever re-pointed at it, since that would silently restore ~16 interruptions per batch. P2 — DISPATCH MEMORY (new pure `shared/cowork-dispatch-memory.ts`): the bulk buttons picked reservations purely from live slot state, so clicking twice during the minutes Cowork takes to attach re-sent the SAME bookings to a second racing task. LOAD-BEARING DESIGN: suppression is keyed on the DISPATCHED UNIT SET, not a timer — a detach re-admits the reservation instantly, because that newly-open slot was never handed to Cowork; the per-kind, position-aware TTL is only the outer bound for the one case no client signal can resolve (a task closed without sending). Storage is localStorage (must be right on FIRST PAINT — a button that renders "(10)" then drops to "(7)" is the very bug) with a `storage` listener so a second tab cannot double-dispatch; counts bind to `ready`, suppression is always visible, and "Send anyway" is a one-click override because a queue that silently SKIPS a booking is worse than one that double-sends. Find cap 8 → 12 (unattended batches only cost a longer run). P3 — THE CARD SITTING: new `buildCoworkBulkCheckoutPrompt` + "Prepare all checkouts in Cowork", one run that walks every attached unbooked unit. It carries THREE batch-only guards without which pressing the per-row button N times is strictly safer, all added after adversarial review found them missing: (1) the checkout claim lane is keyed by reservationId, so reservation 1 at awaiting_payment does NOT block a claim on reservation 2 — the frame requires a machine check against `/api/operations/buy-in-slot-status` over the batch's own ids before EVERY claim, or a batch could open two unsubmitted payment tabs (something no single-row task can do); (2) the last reservation may be prepared an hour after launch and the find-on-VRBO flow re-points costPaid on an existing buy-in, so costPaid must be compared for EQUALITY, not presence, or the 15% guard arms against a stale total; (3) the locked handoff sentence names no booking, so across ~16 handoffs the frame requires an identity line (reservation, guest, unit, total) immediately before it and inside the spoken alert. ALSO FIXED (F4, pre-existing but made likely by P3): a unit stranded at `awaiting_payment` was a dead end — the row's checkout button hides while a checkout is active and `resetBuyInCheckoutClaim` refused anything but queued/in_progress. Reset now accepts `allowAwaitingPayment`, OPT-IN only and behind a confirm that says outright that resetting asserts the payment was never made (resetting a paid unit could buy it twice); never default it, never let an automated path or an agent set it. UNCHANGED AND NOT NEGOTIABLE: the operator enters the card and clicks the final Book/Confirm/Pay on every unit; one outstanding handoff; bulk fills open slots only and never detaches; the server bulk queue stays the phone/detach fallback. Verified: NEW cowork-bulk-split 41/0, cowork-launch 60/0, cowork-buyin-prompt 284/0, cowork-refresh 30/0, full `npm test` REAL exit 0, build clean, `npm run check` 335 = baseline (stash A/B), and both flows proven on the BUILT bundle (Playwright + mocked /api): bookings-cowork-ui 11/0 (no regression from PR #1096) and bulk-split-ui 17/0 — the 2-reservation batch relays as `bulk-find`, covers both reservations, contains no card stop at all, a second click cannot re-dispatch, and "Send anyway" restores it. Could NOT live-run Cowork itself (no session) — post-deploy: select several bookings, press "Auto Cowork bulk · find cheapest", send once, and leave; expect zero further prompts until the units are attached.

2026-07-19 (third PR) · Jamie (screenshot of the empty-slots panel showing BOTH "Auto Cowork · find cheapest" AND "Auto-run · find cheapest (no window)"): "Can you please let me know why there are two buttons? I thought we were now just doing the one button for cowork where the prompt is automatically pushed to claude cowork and starts automatically etc. Please remove the button that does NOT do what we want it to do." · ACCEPTED · WHY THERE WERE TWO: two PRs landed the same day from different sessions — #1101 added the headless `HeadlessFindRunButton` (zero-click: same brief, run by `claude -p` on the Mac, no Cowork window and no send press) and left the older deep-link `CoworkBuyInPromptButton` beside it as a twin. The deep-link one only PRE-FILLS a Cowork task and then waits for a send press, which is the exact friction the whole 2026-07-19 sequence existed to remove, so it was the wrong twin to keep. SHIPPED (`claude/remove-cowork-find-button`): `CoworkBuyInPromptButton` DELETED (component + render + its now-unused `buildCoworkBuyInPrompt` client import); the row's find is now the headless runner alone. `buildCoworkBuyInPrompt` itself is NOT dead and must not be removed — it is still the per-reservation brief the headless runner executes server-side AND the body of every bulk find batch (`buildCoworkBulkBuyInPrompt` composes it). Stale copy repointed in three places that told the operator to "use Auto Cowork · find cheapest" (interrupted-scan notice, lost-background-search toast, the no-slots-configured hint) plus the panel lead-in. UNTOUCHED ON PURPOSE: the four remaining row Cowork prompts (checkout, verify-community, guest-happy, find-on-VRBO) — checkout in particular STAYS a human-gated Cowork prompt, because sending it is the operator's approval to spend money — and BOTH bulk buttons, since there is no headless bulk runner yet and removing them would leave no bulk find at all (the natural follow-up is pointing bulk at the headless runner for genuinely zero-click batches). Test locks repointed preserving intent: cowork-launch now asserts the row has EXACTLY ONE find button and it is the headless one (`button-headless-find-run-` present, `button-cowork-prompt-` and `<CoworkBuyInPromptButton` absent), `useCoworkLaunch` count 6→5 (1 definition + 4 uses), the row-relay-kind guard replaced by "the row find is headless; only BULK still needs the relay" (the `"find"` kind STAYS in the server's ALLOWED_KINDS for backward compatibility — dropping it would 400 a relay run the operator had already launched), and cowork-buyin-prompt's remaining-budget mirror count 3→2 (bulk + headless). Verified: cowork-launch 61/0, cowork-buyin-prompt 284/0, claude-find-run 83/0, full `npm test` REAL exit 0, build clean (bundle-grepped: "Auto Cowork · find cheapest" GONE from the client bundle, "Auto-run · find cheapest" present), `npm run check` 335 = baseline, and the row proven on the BUILT bundle (Playwright + mocked /api, 5/0: panel renders, no "use Auto Cowork" copy, headless button present, deep-link button absent, exactly one find button).

2026-07-19 (fourth PR) · Operator-initiated investigation: the live Menehune Shores listing (draft -3, 6a037d5deaf9eb001bafa2b9) showed SAMPLE tax IDs in its OTA-facing publicDescription.notes ("GE-099-999-9999-99" / "TA-099-999-9999-99"); asked to pull real values, re-push, and audit the rest of the portfolio · DONE (operational fix + `claude/live-sample-license-cleanup`) · ROOT CAUSE: the values were pushed BEFORE the 2026-07-10 placeholder guard existed (the all-9s family is caught today by isPlaceholderLicenseValue's 4-consecutive-9s digit rule, not the sample-core list). PORTFOLIO AUDIT (script: fetch every guesty_property_map listing, classify every compliance surface — notes, tags, top-level licenseNumber/taxId, Booking.com license contentData — with isPlaceholderLicenseValue): 18 of 24 mapped listings carried pre-guard samples; 7 had them in GUEST-VISIBLE notes (-2, -15, 4, 19, 23, 24, 32), the rest only in the Booking.com license object (mostly permit_number). OPERATIONAL FIX (no code needed for the mechanism — it all existed): per flagged property, the deployed pull routes ran with persist=1 (TMK via GIS master parcel, TAT via the Guesty-listing leg — most listings' Booking.com license object already carried a REAL TAT — or the public-listing leg, GET via the paired-TAT derivation, STR via the Kauai TVR / Maui STRH registries), then POST /api/builder/push-compliance re-pushed ONLY real values, which rewrites notes + tags wholesale (samples vanish even when a field found no real replacement). KEY FACTS learned (don't re-derive): (1) positive-id CORE properties are fully covered by the pull machinery — persistPulledComplianceValues writes property_compliance_overrides for them; the static unit-builder-data sample values are only the lookup's UNUSED seed (lookupHawaiiComplianceField destructures propertyValues and never reads it); (2) the integrations[].bookingCom.license read-back is ASYNC channel state — a PUT to channels.bookingCom.license returns 200 but the read keeps showing the old contentData until the next Guesty↔Booking sync, so never diagnose a failed license push from an immediate read-back; (3) top-level licenseNumber/taxId PUTs do NOT persist on this account (push-compliance already reports ok:false honestly — tags/notes/Booking are the real carriers). CODE (this PR): "TVR-2024-099" (live-observed on -25's Booking license object, pushed pre-guard alongside a known sample TAT) added to LEGACY_SAMPLE_LICENSE_VALUES — PROVEN fake: the County of Kauai TVR registry (401 records) issues only TVNC-#### permit numbers and contains no "2024-099" in any field; exact value only, the TVR-YYYY-NNN SHAPE stays real-shaped (TVR-2020-101 false-positive lock untouched). Public-listing-sourced TAT/GET values (e.g. -2's, pulled from another host's listing of the same community) carry their source in license_provenance.v1 — operator should spot-check those against his own tax registrations.

2026-07-19 (fourth PR) · Jamie: "I just ran a prompt, please check the logs and make sure everything is working properly." · FOUND A REAL DEFECT, run cancelled, guard shipped · THE RUN (a66bbde6, Menehune Shores, 19:23Z) started correctly — runner claimed it, dedicated Chrome up on 9250, `claude -p` spawned — but its `system/init` line reported `mcp_servers: [{"name":"chrome","status":"failed"}]` and `tools: NO CHROME TOOLS`. It ran BLIND on WebSearch/WebFetch, and the agent said so itself — "ATTENTION: browser tools missing … I will … attach the best-qualified listings I can verify" — then kept going toward attaching units it had never opened. NOTHING STOPPED IT; the only trace was prose inside a JSONL on the Mac. Cancelled via POST /api/claude-find-runs/:id/cancel; verified zero attach/buy-in API calls and `buy-in-slot-status` still empty for the reservation, so no damage. ROOT CAUSE of the MCP failure: the runner launched `npx -y chrome-devtools-mcp@latest`, which forces a registry round-trip on EVERY run; that cold resolve blew the CLI's MCP startup window (the 13:38 smoke run connected fine, and a manual probe connected fine once warm — it is intermittent, not broken). SHIPPED (`claude/findrun-browser-guard`): (1) THE REAL FIX — new pure `browserMcpFailure()` (runner .mjs) / `browserMcpFailureFromInit()` (shared TS twin, equivalence-locked) reads the init line's mcp_servers; if chrome isn't `connected` the runner kills the CLI immediately and posts a terminal FAILED with an operator-facing explanation. LOAD-BEARING ORDERING: the browserFailure branch sits AHEAD of the sawResult/success branch in the terminal classifier, because a browser-less run can still emit subtype:"success" full of web-searched guesses and that must never be recorded as a completed find. (2) chrome-devtools-mcp PINNED to 1.6.0 (env `CLAUDE_FIND_RUN_CHROME_MCP`) — never restore "@latest"; the npx cache was also pre-warmed on the Mac. (3) `--strict-mcp-config` so a run loads ONLY its own MCP config — a real run had pulled in the operator's user-scope `railway-mcp-server`, which is both needless attack surface for an unattended agent and extra work competing for the same MCP init window. DOCUMENTED SCOPE (verified against the real CLI): "connected" means the MCP PROCESS handshook, NOT that Chrome is reachable — a dead --browserUrl still reports connected. That is acceptable: a reachable-but-broken browser makes every mcp__chrome CALL fail loudly. This guard covers the SILENT case, the tools never existing. Verified: claude-find-run 95/0 (was 83; incl. the twin equivalence lock + 5 runner source guards), full `npm test` REAL exit 0 (the city-vrbo-expansion.smoke load-flake reproduced and passed 8/0 isolated), build clean, `npm run check` 335 = baseline, AND proven END-TO-END against the REAL `claude -p` CLI: with the chrome MCP pointed at a non-existent binary the init line reported `chrome: failed`, the guard fired, killed the CLI, and surfaced the operator message. Live daemon updated (the runner file was identical to origin/main, so a wholesale copy was safe — worker.mjs is the file that runs ahead) + `launchctl kickstart`; runner confirmed back up (pid 31561) with all four guard markers present.

2026-07-19 (fifth PR) · Jamie: "It just said that buy in failed as the browser couldn't be attached." · THE GUARD WAS WORKING — this fix makes the run SUCCEED. The 20:19Z run (aa223f01) hit the #1106 browser guard and failed loudly (correct — no silent degrade). ROOT CAUSE isolated: chrome-devtools-mcp's serverInfo handshake genuinely takes ~2.6s (measured; npx spawn + package load + CDP connect), but the CLI marked chrome "failed" only ~1.5s after claim — the MCP connect was overrunning the CLI's default startup budget on a cold/contended start. Reproduced both ways: my two controlled `claude -p` runs (warm) BOTH showed chrome→connected, confirming it is a timing race, not a hard break; `--strict-mcp-config` from #1106 already removed the competing railway-mcp-server. SHIPPED (`claude/findrun-browser-preflight`), two belts, runtime plumbing (daemon-only, no shared TS twin — it is spawn/timeout logic, not classification): (1) PREFLIGHT — new `preflightBrowserMcp()` runs the EXACT `npx chrome-devtools-mcp@1.6.0 --browserUrl 9250` and waits for the real serverInfo handshake BEFORE the costly agent run; retried `BROWSER_PREFLIGHT_ATTEMPTS` (3) times with an `ensureRunnerChrome()` re-check between attempts; a clean handshake warms the npx cache AND proves Chrome is reachable, so the CLI's connect moments later is fast — this is what makes a transient cold-start SELF-HEAL instead of failing. If all attempts miss, the run ends loudly (never runs browser-less). (2) `MCP_TIMEOUT=30000` in the child env (`CLAUDE_FIND_RUN_MCP_TIMEOUT_MS`) — a real budget for the 2.6s connect under load. The #1106 init-line guard (`browserMcpFailure`) stays as the last-resort net. Verified: claude-find-run 99/0 (was 95; +4 source guards on the preflight loop, fail-closed path, real-handshake check, MCP_TIMEOUT), full `npm test` REAL exit 0, `npm run check` 335 = baseline, build clean, AND proven END-TO-END against the REAL `claude -p` with the runner's exact config (strict + MCP_TIMEOUT + pinned pkg + real CDP 9250): chrome→connected with 29 chrome tools; preflight against live Chrome hand-shook in ~2.6s. Live daemon updated (runner file was identical to origin/main post-#1106, wholesale copy safe; worker.mjs is the one ahead) + kickstart; runner back up (pid 75738). Post-deploy: re-run the prompt — the preflight warms the browser and the run should attach and proceed; a genuinely dead browser still fails loudly after 3 attempts.

2026-07-19 (sixth PR — the ACTUAL root cause) · Jamie re-ran and hit "the runner's browser would not start after 3 attempts". The #1104/#1106/#1107 work all held (guard fires, preflight retries, fails loudly) but the run still could not get a browser. TRUE ROOT CAUSE, proven not guessed: `ps eww` on the LIVE runner (pid 75738) showed `PATH=/usr/bin:/bin:/usr/sbin:/sbin` — launchd does not source a login shell, so the daemon inherits a BARE PATH with no Homebrew/`.local/bin`. `npx` lives at /opt/homebrew/bin (and ~/.local/bin), so `spawn("npx")` dies with ENOENT in ~5ms — for BOTH the CLI's own MCP-server spawn (why every run since #1101 came up chrome=failed) AND the #1107 preflight (why all 3 attempts missed instantly: the daemon log shows attempts 1/2/3 at 20:44:09.699/.706/.714 — 15ms total, i.e. ENOENT, not the ~2.6s handshake). EVERY earlier test PASSED because I ran them from a shell with the full user PATH — the "~2.6s cold-start latency" theory was a shell-side artifact; the real handshake time was never the problem. SHIPPED (`claude/findrun-npx-path`), BOTH parts required: (1) `resolveNpxBin()` → an ABSOLUTE npx (env `CLAUDE_FIND_RUN_NPX_BIN` → `dirname(process.execPath)/npx` → `~/.local/bin/npx` → /opt/homebrew → /usr/local; existsSync-checked) used at BOTH spawn sites — the MCP config `command` and the preflight `spawn`. (2) `EXTENDED_PATH` (runtime's own bin dir + ~/.local/bin + /opt/homebrew/bin + /usr/local/bin + inherited PATH) applied to the preflight spawn env AND the CLI child env — because npx is a `#!/usr/bin/env node` script, so even called by absolute path it needs `node` resolvable on PATH, as does everything npx spawns. #1107's preflight + MCP_TIMEOUT stay (they are correct belts once npx resolves). Verified: claude-find-run 102/0 (+3 guards: absolute-npx resolution, both-sites-use-NPX_BIN via comment-stripped code check, extended-PATH on both envs), full `npm test` REAL exit 0, `npm run check` 335 = baseline, build clean, AND proven END-TO-END UNDER THE DAEMON'S EXACT BARE PATH (`env -i PATH=/usr/bin:/bin:/usr/sbin:/sbin`): the old bare `spawn("npx")` reproduced ENOENT in 5ms; the new resolution hand-shook in 2395ms and the real `claude -p` (strict + MCP_TIMEOUT + absolute npx + extended PATH) came up chrome=connected with 29 chrome tools. Live daemon updated + kickstart; runner up (pid confirmed) and its startup line now logs `npx /opt/homebrew/Cellar/node/25.7.0/bin/npx`. A live re-run was enqueued to confirm on the real daemon before merge. LESSON: for a launchd-spawned daemon, NEVER assume PATH — resolve every external binary absolutely and set a real PATH for children; a shell test cannot catch this class.

2026-07-19 (seventh PR) · Jamie (inbox screenshot: Kavitha Shetty booked Royal Kahana on VRBO on 7/19 for a 7/20 arrival): "In my settings in the pricing tab for this unit it says that we need 7 days notice before a guest can arrive. Please investigate what went wrong. Did Guesty never get pushed this setting etc?" · DIAGNOSED LIVE + FIXED (`claude/booking-rules-advance-notice-gap`) · ROOT CAUSE (three stacked): (1) the Pricing tab's Booking Rules card renders client-side DEFAULT_BOOKING_RULES (7d advance) whenever GET /api/builder/booking-rules returns rules:null, so a never-pushed listing READS as protected; (2) the advance-notice cutoff was pushed portfolio-wide exactly ONCE — 2026-06-02, push-advance-notice-all, 12 rows stamped seconds apart — and NO mapping-birth hook existed for booking rules (unlike amenities + published address), so all 12 listings mapped after Jun 2 had NO cutoff; (3) prod-verified: Royal Kahana (draft 46 → listing 6a3bdba43b75bc0023cd9230) had calendarRules.advanceNotice with no defaultSettings.hours and expedia.isCutOffHoursEnabled:false → VRBO (homeaway2) accepted the reservation (created 2026-07-19T20:03:57Z, check-in 7/20 HST, $6,711 fare, confirmed) ~19h before arrival. FIX: autoPushBookingRulesForMapping at all five mapping-birth seams (skip-if-row-exists, live-terms merge, 7d forced, min-nights floored at DEFAULT_MIN_NIGHTS, 2-min cooldown, kill BOOKING_RULES_AUTO_PUSH_DISABLED=1); push-advance-notice-all now FLOORS min-nights (its saved rows still carry the 2026-06-02 era's minNights=3 — a verbatim re-run would have LOWERED the live min-4 policy); amber "Never pushed to Guesty" chip on the Booking Rules card keyed off confirmed rules:null. See the "Booking rules (advance notice) auto-push at mapping birth" Load-Bearing section. Post-merge live step: run the bulk endpoint against the deployed portal to backfill the 12 exposed listings and re-verify 168h on each.

2026-07-20 · Jamie (screenshot, both units attached but "$0 · Jul 21 → Jul 26"): "when the Cowork attached the buy ins it didn't have any pricing attached." · REAL DEFECT (the #1108 npx/PATH fix worked — the run attached both slots; this is the NEXT bug). ROOT CAUSE, proven from the run transcript (c1110e0e): the agent FOUND real prices (Maui Banyan A-202 $1,400 / Luana Kai C307 $1,968) but its create-buy-in curls hill-climbed against the endpoint's 422s — `{}` → `{unitId}` → `{unitId,listingUrl}` (wrong field name) → `{unitId,airbnbListingUrl}` (200) — and DROPPED costPaid, unitLabel, unitAddress, notes along the way. The `/api/claude-find-runs/agent/:id/buy-ins` proxy 422'd on missing unitId + invalid airbnbListingUrl but SILENTLY defaulted a missing costPaid to "0", so the agent's minimal accepted body created two $0 buy-ins — and a $0 costPaid breaks the profit calc AND the checkout 15% guard, which anchor on it. SHIPPED (`claude/findrun-buyin-pricing`): (1) THE FIX — the proxy now REQUIRES costPaid, parses it tolerantly (strips $ , whitespace so "$1,400.00" and "1,400" both work — the agent narrates prices that way; rejecting a real price it DID send is worse than accepting a formatted one), and 422s with a clear message when it is missing/non-numeric/≤0, exactly as hard a requirement as unitId/URL — so the agent's hill-climb lands on INCLUDING it. The old `: "0"` silent default is gone; stored value is `costPaid.toFixed(2)`. (2) Belt-and-braces prompt: the HEADLESS-ONLY auth note now says "Send the COMPLETE create body in ONE call" + costPaid REQUIRED > 0 + "recorded at 0 is a bug" — headless-only, so the default-prompt byte-identical lock is untouched. LESSON worth keeping: an agent that hill-climbs against error messages will DROP any field the server accepts when absent; every field that matters must be a hard 422, not a silent default. Verified: claude-find-run 104/0 (+2: server rejects missing/zero costPaid, prompt demands complete body), cowork-buyin-prompt 284/0 (byte-identical lock intact), full `npm test` REAL exit 0, `npm run check` 335 = baseline, build clean, and the exact agent body that caused the $0 (`{unitId,airbnbListingUrl}`) proven rejected in isolation while "$1,400.00"/"1,400"/1968 are accepted and "0"/"abc" rejected. The two live $0 buy-ins (537 Maui Banyan A-202, 538 Luana Kai C307) were corrected to their real transcript prices post-fix.

2026-07-20 · Jamie: "Is there a way to confirm that the unit is what the guest expected? I think we actually created a guest expectation button so after cowork attaches can it then run the guest expectation button automatically?" · Confirmed the button exists (`CoworkGuestHappyButton` / `buildCoworkGuestHappyPrompt` — the "Will guest be happy?" evaluation: original booked listing vs attached units across community / size / bedding layout / photo quality → happy|concerns|unhappy verdict recorded on every attached buy-in, surfaced as ★/⚠/✕ badges). Operator chose (AskUserQuestion): run it AUTOMATICALLY after every headless find-run attach, and a concerns-OR-unhappy verdict must ALERT loudly + mark the badge (never auto-detach). SHIPPED (`claude/findrun-guest-expectation`), server-only (no runner .mjs change — the agent runs the new phase from the brief and the ATTENTION-marker chime is already wired): (1) NEW `afterAttach: "guest_expectation"` opt on buildCoworkBuyInPrompt appends a Phase 2 to the headless find brief (gated `&& !!headless` — inert without it, and the DEFAULT/no-opts prompt stays byte-identical, test-locked). Phase 2 is READ-ONLY apart from the verdict: study the original booked listing, study each attached unit with the run's browser VISION, compare dimension-by-dimension, record the verdict, and — on concerns/unhappy — print an `ATTENTION: Guest expectation — <reason>` line (the daemon's existing marker scan fires the Sosumi alarm + portal attention) but DO NOT wait: finish the report and end normally. Never books/detaches/re-finds. (2) The server always builds the headless brief with this opt now. (3) NEW run-scoped proxy `POST /api/claude-find-runs/agent/:id/guest-happy` (mirrors the buy-ins/attach proxies — run-token auth, reservation PINNED from the run record never the body, source forced "cowork", forwarded via loopback to the existing `/api/bookings/:id/guest-happy` which stamps every attached buy-in). The headless agent never holds ADMIN_SECRET. The manual "Will guest be happy?" button stays for re-checking. LESSON reused: the headless find-run's afterAttach mechanism (built for checkout-prep chaining) generalizes cleanly to any read-only post-attach phase. Verified: claude-find-run 113/0 (+9: phase render, run-scoped proxy + no raw endpoint, three-verdict enum, alert-but-don't-wait, read-only, inert-without-headless, server passes the opt, proxy pins reservation + forces source), cowork-buyin-prompt 284/0 (byte-identical + prepare_checkout paths intact), full `npm test` REAL exit 0, `npm run check` 335 = baseline, build clean. Post-deploy: the run-scoped proxy live-verified (rejects a bad verdict 422; a valid verdict reaches the underlying endpoint) on a throwaway probe run — no real verdict fabricated (that requires the agent's actual vision comparison).

2026-07-20 · Jamie (screenshot of the Headless find-run panel — a flat monospace wall of "Browser: close page" lines, the final report leaking in as a truncated "21:34 · --- ## Final Report …" LOG line AND repeated verbatim below in a raw-markdown <pre> with visible pipes/##/**): "Please improve the UI for the logs. Separate each session better and show needed information better." · SHIPPED (`claude/findrun-log-ui`) a full redesign of HeadlessFindRunPanel + supporting pure/server plumbing: (1) NEW pure `shared/claude-find-run-display.ts` (33 tests) — `classifyFindRunEvent` (milestone | search | browse | note | lifecycle | error; the three portal writes are milestones, browse chatter is `dim`), `isFinalReportEcho` (the runner captures the final message as a 400-char note too — this flags that echo so the feed drops it, killing the double-render), and `parseFindRunReport` (agent markdown → a SMALL SAFE block grammar: heading / paragraph / list / pipe-table / hr + inline **bold**; returns descriptors, never an HTML string, so agent-authored text has no injection surface). (2) The panel now has: a header with status pill + started time + duration (`formatFindRunDuration`) + cancel; an OUTCOME strip (units attached + prices summed + the guest-expectation verdict badge, derived from the reservation's attached buy-ins passed in via a new `attachedUnits` prop — `findRunGuestVerdictBadge`); a TYPED activity feed (`FindRunEventRow` — green ✓ milestones as section markers, dimmed globe for browse noise, quoted italics for the agent's notes, the report-echo filtered out); the report RENDERED as real elements (`FindRunReport` → table/bold/lists, no `<pre>` dump); and a collapsed "Earlier runs (N)" list — the "separate each session" ask. (3) `/api/claude-find-runs/status` now returns a compact per-reservation `history` (prior runs: id/status/times, newest-first, capped 8, no events/report to keep the fan-out payload small — `claudeFindRunHistoryForReservation`). (4) describeToolUse (runner .mjs + shared twin) maps the guest-happy curl → "Recording the guest-expectation verdict" so Phase 2 shows as a milestone. LOAD-BEARING: the default/no-opts find prompt stays byte-identical (untouched); the report parser is a deliberately minimal grammar (unrecognised lines fall through as paragraphs, never dropped, never HTML). Verified: claude-find-run-display 33/0, claude-find-run 122/0 (+9 wiring guards), full `npm test` REAL exit 0, `npm run check` 335 = baseline, build clean, AND the redesign proven on the BUILT bundle (Playwright + mocked /api, 13/0: outcome strip + prices + verdict badge + duration, report rendered as a real table with no literal pipes/##/**, the duplicate "## Final Report" feed line GONE, milestones present, "Earlier runs (2)" listing the failed + cancelled prior sessions) + a screenshot spot-check. Live daemon updated for the milestone label (runner file was == origin/main; wholesale copy + kickstart).

2026-07-20 · Jamie (screenshot of the Jacelyn Tsu row, both units attached): "Shouldn't there be a button for checkout via cowork for each unit? As each unit I will need to check out individually." · SHIPPED (`claude/per-unit-checkout`), client-only: the expanded row now renders ONE "Prepare checkout" button PER attached unbooked unit instead of a single button that embedded every unbooked buy-in in one combined brief. Each button builds `buildCoworkCheckoutPrompt` with a SINGLE-element `units` array (the builder has always handled n=1 — no shared/ change), so each unit runs as its own Cowork task with its own card handoff. Multi-unit rows label each button "Prepare checkout · <slot label>" with testid `button-cowork-checkout-prompt-<resId>-<buyInId>`; a row with exactly ONE unbooked unit keeps the classic "Prepare checkout in Cowork" label + unsuffixed testid (nothing the operator or a test knows by name changes shape). LOAD-BEARING, unchanged on purpose: (1) ALL per-unit buttons hide while ANY checkout on the reservation is queued/in-progress/awaiting_payment (`buyInHasActiveCheckout` gate) — the reservation-scoped checkout claim allows exactly one outstanding handoff, so a sibling launch would just 409 at the claim; finish or release the active unit and the next unit's button reappears. (2) The costPaid freshness pre-flight compares ONLY the buy-ins the click sends (`sentIds`) — with a one-element scope this is naturally per-unit. (3) The bulk "sit the cards" checkout batch (`button-run-bulk-cowork-checkout`) deliberately keeps the combined per-reservation brief — one Cowork task walking units in order is the point of the batch. Verified: cowork-refresh 33/0 (+3 source guards: per-unit map + single-element scoping, label/testid shapes, gate wraps the map), cowork-buyin-prompt 284/0 + cowork-launch 61/0 + cowork-bulk-split 50/0 untouched, full `npm test` REAL exit 0, `npm run check` 335 = baseline, build clean, AND proven on the BUILT bundle (Playwright + mocked /api, 12/0: two labeled buttons render; clicking Unit B copies a brief with buyInId 538 + $1968.00 and WITHOUT the sibling; clicking Unit A copies its own; money guards present in each brief; solo row keeps classic label/testid; booked sibling buttonless; awaiting_payment row hides all buttons).

2026-07-20 · Jamie (after the per-unit checkout buttons shipped): "There are now two buttons for cowork but it doesn't start cowork automatically like it does when it auto fills the two cheapest buy in units. Please update this so it executes cowork automatically." · ACCEPTED — HEADLESS PER-UNIT CHECKOUT RUNS (`claude/headless-checkout`). This UPDATES the 2026-07-19 "FIND-ONLY, forever" boundary (replaced in the Load-Bearing section, not deleted): the money approval that rule protected was the Cowork send press; it is now the CARD ITSELF, which never left human hands. The per-unit "Prepare checkout" deep-link buttons became "Auto checkout" buttons that POST the NEW `POST /api/claude-find-runs/checkout` — the same headless runner infrastructure as the find flow, kind:"checkout", one run per pinned buy-in. SERVER-AUTHORITATIVE MONEY DATA: the endpoint reads the buy_ins row via loopback and gates on NEW pure `checkoutRunEligibility` (shared/claude-find-run.ts — attached to THIS reservation; not booked/request_submitted [duplicate-purchase risk]; not queued/in_progress/awaiting_payment [claim lane active]; https vrbo.com host exactly [non-VRBO → "Find property on VRBO" first]; costPaid > 0 [anchors the 15% guard]) — the client sends identity data only (guestName REQUIRED full name, propertyName, party), which RETIRED the old client-side costPaid freshness pre-flight along with `CoworkCheckoutPromptButton` itself. BRIEF: `buildCoworkCheckoutPrompt` gained the same `headlessRun` opt as the find builder (defaults byte-identical — proven old-vs-new across ALL non-headless variants incl. bulk + find-and-prepare via a dump-diff; the bulk "Prepare all checkouts" deep-link path is untouched): endpoints swap to run-scoped proxies, HEADLESS_BOT_WALL_PROTOCOL replaces the Cowork sounds, and the tail changes from wait-for-my-click to: complete call records awaiting_payment → print `ATTENTION: awaiting payment — <unit>` → KEEP the prepared checkout tab OPEN (close only its other tabs) → END with the report; the run never enters card data, never clicks the final purchase control, and NEVER writes booked/request_submitted. PROXIES (server/claude-find-runs.ts): `GET agent/:id/buy-in`, `POST agent/:id/traveler-email`, `POST agent/:id/checkout-claim{,/complete,/release}` — kind-gated, reservation AND buyInId pinned from the run record, claimToken/guest-name/reason are the only agent-supplied fields, and there is deliberately NO bookingStatus proxy. Cross-kind single-flight: one live run per reservation (a find mutating slots under a checkout prep is the interleaving this blocks). RUNNER: describeToolUse twins (.mjs + shared, equivalence-locked) label the five checkout milestones ("Recording the payment handoff — awaiting your card" etc.), and NEW `surfaceRunnerChrome()` — CDP Browser.setWindowBounds over global WebSocket, chrome-sidecar-manager's technique — fires on EVERY attention raise, so the hidden runner Chrome pops on-screen for bot walls AND the card handoff (find runs benefit too; never re-hidden). CLOSING THE LOOP: headless runs end at the handoff with no chat to record the outcome, so the awaiting_payment badge gained "Paid — mark booked" (prompt for the confirmation number → PATCH booked; type REQUEST → request_submitted; blank → confirm-gated booked-without-number) beside the existing "Not paid — reset" — this also fixed the pre-existing dead end where a PAID-but-Cowork-abandoned unit had no recovery at all. Panel/history label checkout runs ("Headless checkout", kind chips, "Running — preparing the checkout"). Verified: claude-find-run 153/0 (+31: eligibility matrix, headless-brief guards incl. plain-brief-untouched, twin milestone equivalence, proxy pinning + no-bookingStatus-proxy + surfacing source guards), cowork-refresh 31/0 (pre-flight guards REPOINTED to the server-authoritative build), cowork-launch 61/0 (hook count 5→4), cowork-buyin-prompt 284/0 untouched, full `npm test` REAL exit 0, `npm run check` 335 = baseline, build clean (labels bundle-grepped; old row label gone), AND proven on the BUILT bundle (Playwright + mocked /api, 13/0): two "Auto checkout · Unit X" buttons; clicking Unit B POSTs {reservationId, buyInId:538} with NO money fields; the panel flips to "Headless checkout · Running — preparing the checkout" with the claim milestone; both buttons disable while live; awaiting_payment shows both exits and "Paid — mark booked" PATCHes booked + confirmation. Post-merge: live daemon runner updated (cp + kickstart). Could NOT live-run a real checkout in-session (that spends a claim on a real reservation) — post-deploy: click "Auto checkout" on an attached unbooked unit; expect the run panel, then the Sosumi chime + the runner Chrome popping up with the prepared VRBO payment page, card fields empty.

2026-07-20 · Jamie: "when Cowork has the unit at the checkout stage when it has created the alias email, used that and the guest first and last name, and then got the checkout page where the credit card part is required, please have it pop up Chrome so I can see it in like a yellow pop up." · ACCEPTED — THE YELLOW CARD-HANDOFF POP-UP (`claude/checkout-yellow-popup`, runner-only). The headless checkout run already surfaced the runner Chrome plainly at the handoff; now the payment stage gets the sidecar's full yellow challenge treatment, painted onto the PREPARED CHECKOUT TAB itself. NEW `surfaceCheckoutHandoff(reason)` in claude-find-runner.mjs: picks the checkout tab from /json/list (vrbo.com host required, checkout/payment/book-shaped URL preferred — a bystander tab is never painted), fronts it via the plain HTTP `/json/activate/<targetId>`, un-minimizes + near-fullscreens the window (CDP Browser.setWindowBounds over global WebSocket), then injects — via raw CDP Runtime.evaluate on the page target's own websocket (this runner has no Playwright) — the exact visual language of worker.mjs `setVrboChallengeHighlight`: fixed top banner "💳 ADD THE CREDIT CARD — <reason> · Card fields are EMPTY; add the card and click Checkout yourself. No purchase has been submitted." (#fde047 on #f59e0b) + a full-viewport #facc15 border, ids `rct-findrun-card-banner/-border/-style`, idempotent re-injection. ROUTING: startAttentionAlarm branches on `/^awaiting payment\b/i` — the payment handoff gets the yellow treatment, every other attention (bot walls) keeps the plain `surfaceRunnerChrome()`; the match is DRIFT-LOCKED behaviorally against the headless brief's mandated "ATTENTION: awaiting payment — …" line (build the brief → scanMarkers → regex must match; reword either side alone and the suite trips). LOAD-BEARING, test-locked: the injection is DISPLAY-ONLY — pointer-events:none border, no querySelector on inputs, no value assignment, no click/submit — the card and the final Checkout click stay the operator's alone, and the banner disappears naturally on the navigation their click causes. Verified: claude-find-run 157/0 (+4), full `npm test` REAL exit 0, `npm run check` 335 = baseline, AND behaviorally proven pre-merge against a REAL CDP Chrome (Playwright-launched, route-mocked vrbo.com checkout + bystander tab, then the exported function called cold: banner painted into the checkout tab only, computed background rgb(253,224,71), border click-transparent, card input untouched, bystander clean, re-surface never duplicates — 8/0). Post-merge: live daemon runner updated (cp + kickstart). Server/client untouched.

2026-07-19 · Jamie (bookings-row screenshot, Menehune Shores 2-unit combo): "The email alias for both of the buy in units is the exact same, they need to be different. … having the VRBO Guest Thread email alias and then right above it the other alias … remove one of them and/or merge them together … Each unit needs to have one email alias to get booking confirmations from and/or reply to the property manager for the arrival details" · ACCEPTED — UNIFIED PER-UNIT ALIAS (`claude/email-alias-dedup-units-af17f9`) · Root cause was TWO independent SimpleLogin alias systems per unit: the unit-scoped reservation_aliases engine (guest.name.<res6>.<unitToken>@ — correctly per-unit) and the per-guest buy_ins.travelerEmail scheme (firstname.lastname@ — its unitEmailIndexForBuyIn raced sibling attaches and its "already exists" catch SILENTLY stored one shared address on two buy-ins; no test locked the collision). SHIPPED: getOrCreateReservationAlias + expiry helpers extracted VERBATIM routes.ts → server/reservation-alias.ts; ensureTravelerEmailForBuyIn now mints/reuses THAT (reservationId, buyInId) alias (legacy scheme deleted); alias mint/lookup backfills travelerEmail (never clobbers); unbooked-and-shared travelerEmail re-mints via pure travelerEmailNeedsRemint (booked units keep their address — VRBO has it); bookings row renders ONE merged panel (BuyInGuestThreadPanel folded into BuyInVendorEmailPanel, history = mergeAliasThread dedupe across buy_in_emails ∪ guest_inbox_messages since both IMAP ingesters watch the same mailbox for the same address; legacy differing travelerEmail stays visible as a "booking email (legacy)" badge). See Load-Bearing #53 "ONE ALIAS PER ATTACHED UNIT". Verified: tests/unified-buyin-alias.test.ts 25/0 (npm chain), pipeline-logic lock repointed, full `npm test` REAL exit 0, build clean ("VRBO guest thread" grepped GONE from the bundle, new labels present), `npm run check` 335 = baseline (stash A/B identical per-file sets).
