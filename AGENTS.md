...
The panel renders for BOTH roles (agents use it in the portal); `canDelete={isAdmin}`
hides the trash button for agents. Pure status/severity rules + labels live in
`shared/guest-issue-logic.ts` so server and client never drift.
