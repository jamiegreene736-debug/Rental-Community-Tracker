...
51. **`module.externalId` (or status `completed`/`sent`/`delivered`) is the ONLY
    ...
    - **`sendGuestyConversationMessage` sends ONCE per successful POST** — it only
      ...
      pre-send idempotency check (`classifyExistingSend`: reuse an already-delivered
      copy; resume-poll a <4-min pending duplicate instead of resending) and returns
