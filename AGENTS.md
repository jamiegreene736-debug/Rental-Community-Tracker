...
2026-06-10 · Jamie asked that running a market-rate update (the dashboard bulk-refresh queue) show the PREVIOUS scan's rate in each pricing-table cell, as a quick scan-over-scan reference · ACCEPTED · `storage.upsertPropertyMarketRate` DELETEs + re-INSERTs the (propertyId,bedrooms) row on every scan, so it now reads the prior row FIRST and copies each month's prior `medianNightly` forward into the new `monthlyRates[ym].previousMedianNightly` (+ `previousRefreshedAt`) via the pure, unit-tested `annotatePreviousMonthlyRates` (`shared/market-rate-history.ts`). LOAD-BEARING: (1) it's jsonb-only — NO migration; the previous value rides inside `monthlyRates` through the existing read/`sanitizeMarketRateRows`(spread-preserving)/client `parseMonthlyRates` pipeline. (2) "previous" = the prior row's CURRENT median, NEVER its own `previousMedianNightly` — no chaining. (3) ALL scan paths funnel through this single chokepoint (bulk queue, single refresh, the 3 routes upserts, hybrid-pricing), so don't add a second writer that bypasses it. Surfaced as a small "was $X (▲/▼ Y%)" line in the builder Pricing-tab 24-month table buy-in cell (`GuestyListingBuilder/index.tsx`), green when the buy-in DROPPED (cheaper to acquire). Client getter `getLivePreviousMonthlyBuyIn` added. Verified: `tests/market-rate-history.test.ts` 10/10 (incl. no-chaining + first-scan + rounding), `npm run build` clean, tsc no new errors in touched files. Couldn't e2e against live DB (unreachable in session) — the previous value appears on the SECOND scan onward (first scan has no prior).
```
