...
CMD ["sh", "-c", "if [ \"${RAILWAY_SERVICE_ROLE:-web}\" = \"sidecar-worker\" ]; then export SIDECAR_BROWSER_MODE=\"${SIDECAR_BROWSER_MODE:-server}\" CHROME_PRIMARY=\"${CHROME_PRIMARY:-server}\" SIDECAR_WORKER_ROLE=\"${SIDECAR_WORKER_ROLE:-server}\" SIDECAR_DISABLE_LOCAL_CDP_FALLBACK=\"${SIDECAR_DISABLE_LOCAL_CDP_FALLBACK:-1}\" SIDECAR_HEADLESS_FALLBACK_ENABLED=\"${SIDECAR_HEADLESS_FALLBACK_ENABLED:-0}\" SERVER_CHROME_FALLBACK_ENABLED=\"${SERVER_CHROME_FALLBACK_ENABLED:-1}\" SERVER_CHROME_FALLBACK_VRBO=\"${SERVER_CHROME_FALLBACK_VRBO:-1}\" SIDECAR_OPEN_NOVNC_ON_ACQUIRE=\"${SIDECAR_OPEN_NOVNC_ON_ACQUIRE:-0}\" MAX_LOCAL_CHROME_INSTANCES=\"${MAX_LOCAL_CHROME_INSTANCES:-1}\"; echo 'Starting Railway sidecar worker role'; exec node daemon/vrbo-sidecar/supervisor.mjs; fi; mkdir -p /app/client/public/photos && cp -Rn /app/photos-seed/. /app/client/public/photos/ 2>/dev/null || true; npm run db:push && exec node dist/index.cjs"]
